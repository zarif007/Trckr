...
} from "@/app/components/tracker-display";
import {
  EditModeUndoButton,
...
function OwnerScopeSettingsReadout({
  banner,
}: {
  banner: OwnerScopeSettingsBanner;
}) {
  const title =
    banner.source === "project" ? "Project settings" : "Module settings";
  ...
  return (
    ...
    >
      ...
      <p className="text-[11px] text-muted-foreground mt-1 leading-relaxed">
        Stored on this {banner.source} in the database (
        <code className="text-[10px]">settings</code> column), separate from the
        tracker form below.
      </p>
      {scopeHint ? (
        <p className="text-xs text-foreground mt-2">
          Default master data scope:{" "}
          <span className="font-medium">{scopeHint}</span>
        </p>
      ) : null}
      {banner.settings == null ? (
        <p className="text-xs text-muted-foreground mt-2">
          No JSON is stored yet.
        </p>
      ) : (
...
export const TrackerPanel = memo(function TrackerPanel({
  ...
  const [dataSnapshot, setDataSnapshot] = useState<Record<
    string,
  ...
  const parseImportJson = useCallback(
    (raw: string): GridDataSnapshot | string => {
      try {
        ...
        for (const [key, val] of Object.entries(parsed)) {
          if (!Array.isArray(val))
            return `Value for "${key}" must be an array of rows`;
        }
    ...
    >
      ...
      >
        {allowSchemaEditToggle && (
          <div
            className={`inline-flex shrink-0 items-center rounded-sm border border-border/40 bg-muted/20 p-0.5 ${isStreamingTracker ? "opacity-60 pointer-events-none" : ""}`}
          >
            {hideChatToggle ? (
              <>
                <button
        ...
        {versionControl &&
          ...
          onVcMergedToMain && (
            <Button
              ...
              aria-label={
                vcDrawerOpen ? "Close version control" : "Open version control"
              }
            >
              <GitBranch className="h-3.5 w-3.5" />
              <span className="hidden sm:inline">
                {vcCurrentBranch?.branchName ?? "Branches"}
              </span>
            </Button>
          )}
      ...
      <Dialog
        open={debugView !== null}
        onOpenChange={(open) => !open && setDebugView(null)}
      >
        <DialogContent
          className={`max-w-2xl ${debugView === "data" && onImportData ? "max-h-[90vh]" : "max-h-[85vh]"} flex flex-col`}
        >
          <DialogHeader>
        ...
        <TrackerDisplayErrorBoundary key={displayKey}>
          {isStreamingTracker ? (
            <TrackerDisplay
              ...
              sections={
                ((schema.sections || []) as unknown[]).filter(
              ...
              grids={
                ((schema.grids || []) as unknown[]).filter(
              ...
              fields={
                ((schema.fields || []) as unknown[]).filter(
                  (f): f is TrackerResponse["fields"][number] =>
                    !!f &&
                    typeof f === "object" &&
                    "ui" in f &&
                    !!(f as { ui?: { label?: string } }).ui?.label,
                ) as TrackerResponse["fields"]
              }
              layoutNodes={
                (schema.layoutNodes || []) as TrackerResponse["layoutNodes"]
              }
              bindings={(schema.bindings || {}) as TrackerResponse["bindings"]}
              validations={
                (schema.validations || {}) as TrackerResponse["validations"]
              }
              calculations={
              ...
              dynamicOptions={
                (schema.dynamicOptions ||
                  {}) as TrackerResponse["dynamicOptions"]
              }
