...
function OwnerScopeSettingsReadout({ banner }: { banner: OwnerScopeSettingsBanner }) {
  ...
  return (
    <div
      className="mb-3 rounded-md border border-border/60 bg-muted/20 px-3 py-2.5"
      role="region"
      aria-label={title}
    >
      ...
      {banner.settings == null ? (
        ...
        <pre className="mt-2 max-h-48 overflow-auto rounded-md border border-border/50 bg-muted/40 p-2.5 text-[11px] font-mono leading-relaxed whitespace-pre-wrap break-words">
        ...
        <TrackerDisplayErrorBoundary key={displayKey}>
          {isStreamingTracker ? (
            <TrackerDisplay
              ...
              fieldRules={(schema.fieldRules || []) as TrackerResponse['fieldRules']}
