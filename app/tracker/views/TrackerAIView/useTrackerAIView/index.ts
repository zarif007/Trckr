...
export function useTrackerAIView(props: TrackerEditorViewProps = {}) {
  ...
  const builderMessagesRef = useRef<{
    messages: Message[];
  ...
  const analyst = useAnalystChat({
    ...
    trackerSchema: (initialSchema ??
      activeTrackerData) as TrackerResponse | null,
    trackerDataRef,
    onConversationCreate:
  ...
  analystMessagesRef.current = {
    messages: analyst.messages,
    setMessages: analyst.setMessages,
  };
  ...
  const [schema, setSchema] = useState<TrackerResponse>(() =>
    normalizeTrackerSchema(
      (initialSchema ?? INITIAL_TRACKER_SCHEMA) as TrackerResponse,
    ),
  );
  const [viewingMessageIndex, setViewingMessageIndex] = useState<number | null>(
    null,
  );
  const [loadedSnapshot, setLoadedSnapshot] = useState<LoadedSnapshot | null>(
    null,
  );
  const [currentFormStatus, setCurrentFormStatus] = useState<string | null>(
    initialFormStatus,
  );
  ...
  const [lastSyncedTracker, setLastSyncedTracker] =
    useState<TrackerResponse | null>(null);
  ...
  const setLoadedSnapshotFromSave = useCallback(
    (s: LoadedSnapshot | null) => {
      setLoadedSnapshot(s);
  ...
  const { formActions, effectiveCurrentFormStatus, isReadOnly } =
    formActionsState;
  ...
  const allowAutoSave =
    ...
    pageMode === "data" &&
    !isReadOnly;
  const allowSchemaAutoSave =
    schemaAutoSave && pageMode === "schema" && Boolean(onSaveTracker);
  ...
  const trackerName =
    schema?.name ?? schema?.tabs?.[0]?.name ?? "Untitled tracker";
  ...
  const formActionsChangeStable = useCallback(
    (actions: TrackerFormAction[]) => {
  ...
  const formActionSelectStable = useCallback((action: TrackerFormAction) => {
  ...
  useEffect(() => {
    if (!setSaveState) return;
    const showManualSaveData =
      allowSaveData && !allowAutoSave && !allowSchemaAutoSave;
    ...
    setSaveState({
      ...
      onFormActionSelect:
        isDataPage && showFormActions ? formActionSelectStable : null,
