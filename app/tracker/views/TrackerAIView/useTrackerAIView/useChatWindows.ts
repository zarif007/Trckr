...
export function useChatWindows(
  ...
  builderMessagesRef: React.MutableRefObject<{
    messages: Message[];
    setMessages: (m: Message[] | ((prev: Message[]) => Message[])) => void;
  }>,
  analystMessagesRef: React.MutableRefObject<{
    messages: Message[];
  ...
  const [analystConversationLoading, setAnalystConversationLoading] =
    useState(false);
  ...
  const onBuilderConversationCreate = useCallback(
    async (
      ...
      if (!trackerId || !activeBuilderWindowId?.startsWith("draft-"))
        return null;
      const title = firstWords(userMessage, 5);
      try {
        const created = await createConversation(trackerId, "BUILDER", title);
        await persistMessage(created.id, {
          role: "USER",
          content: userMessage,
        });
        const draftId = activeBuilderWindowId;
        setBuilderChatWindows((prev) =>
          prev.map((w) => (w.id === draftId ? { id: created.id, title } : w)),
        );
        setActiveBuilderWindowId(created.id);
        builderWindowStateRef.current[created.id] = builderWindowStateRef
          .current[draftId] ?? { messages: [] };
  ...
  const onAnalystConversationCreate = useCallback(
    async (
      ...
      if (!trackerId || !activeAnalystWindowId?.startsWith("draft-"))
        return null;
      const title = firstWords(userMessage, 5);
      try {
        ...
        analystWindowStateRef.current[created.id] = analystWindowStateRef
          .current[draftId] ?? { messages: [] };
        return { id: created.id, title: created.title ?? title };
      } catch {
        return null;
      ...
      try {
        ...
        if (list.length === 0) {
          ...
          setBuilderChatWindows(
            list.map((c) => ({
              id: c.id,
              title: conversationDisplayTitle(c.title),
            })),
          );
          const hasInitial =
            initialConversationId &&
            list.some((c) => c.id === initialConversationId);
          if (hasInitial && initialConversationId)
            setActiveBuilderWindowId(initialConversationId);
          else
            setActiveBuilderWindowId((prev) =>
      ...
      try {
        ...
        if (list.length === 0) {
          ...
          const hasInitial =
            ...
            list.some((c) => c.id === initialConversationId);
          if (hasInitial && initialConversationId)
            setActiveAnalystWindowId(initialConversationId);
          else
            setActiveAnalystWindowId((prev) =>
              list.some((c) => c.id === prev) ? prev : list[0].id,
            );
        }
      } catch {
