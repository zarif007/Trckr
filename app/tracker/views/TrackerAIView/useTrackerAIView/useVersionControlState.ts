...
export function useVersionControlState(
  ...
  const [vcCurrentBranch, setVcCurrentBranch] = useState<BranchRecord | null>(
    null,
  );
    ...
    async function fetchBranches() {
      try {
        ...
        const selected =
          (initialBranchName &&
            branches.find(
              (b) => b.branchName === initialBranchName && !b.isMerged,
            )) ??
        ...
        if (selected) {
          ...
          if (selected.data && typeof selected.data === "object") {
            setLoadedSnapshot({
              ...
              formStatus:
                (selected as BranchRecord & { formStatus?: string | null })
                  .formStatus ?? null,
            });
          }
        }
    ...
    (branch: BranchRecord) => {
      ...
      if (branch.data && typeof branch.data === "object") {
        const snapshot: LoadedSnapshot = {
          ...
          formStatus:
            (branch as BranchRecord & { formStatus?: string | null })
              .formStatus ?? null,
        };
    ...
    (updatedMain: BranchRecord) => {
      setVcBranches((prev) =>
        prev.map((b) => {
          ...
          if (b.id === vcCurrentBranchRef.current?.id)
            return { ...b, isMerged: true };
          return b;
        }),
      );
