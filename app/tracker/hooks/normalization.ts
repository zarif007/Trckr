...
export function isUntouchedFirstRunScaffold(
  tracker: TrackerLike | null | undefined,
): boolean {
  if (!tracker) return true;
  const trackerWithExtras = tracker as TrackerLike & {
    styles?: unknown;
    dynamicOptions?: unknown;
  };
  const scope = normalizeMasterDataScope(
    (tracker as { masterDataScope?: unknown }).masterDataScope,
  );
  ...
  const layoutNodes = Array.isArray(tracker.layoutNodes)
    ? tracker.layoutNodes
    : [];
  const fieldRules = Array.isArray(tracker.fieldRules)
    ? tracker.fieldRules
    : [];
  ...
  if (
    ...
    !isEmptyObject(
      (tracker as TrackerLike & { calculations?: unknown }).calculations,
    ) ||
...
export function normalizeValidationAndCalculations(
  tracker: TrackerLike,
): TrackerLike {
  ...
  for (const n of layoutNodes) {
    ...
    if (!gridsByFieldId.has(n.fieldId))
      gridsByFieldId.set(n.fieldId, new Set());
  ...
  const coerceRules = (value: unknown): FieldValidationRule[] => {
    ...
    if (value && typeof value === "object")
      return [value as FieldValidationRule];
    return [];
  };
