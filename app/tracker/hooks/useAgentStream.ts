...
function deriveBuilderStatus(partial: Partial<BuilderOutput>): string {
  ...
  const tracker = (partial.tracker ?? partial.trackerPatch) as Record<string, unknown> | undefined
  ...
  if (validations && Object.keys(validations).length > 0) return 'Adding validation rules...'
  if (calculations && Object.keys(calculations).length > 0) return 'Setting up calculations...'
  if (bindings && Object.keys(bindings).length > 0) return 'Wiring data bindings...'
  if (layoutNodes && layoutNodes.length > 0) return 'Arranging layout...'
  if (fields && fields.length > 0) return `Defining fields (${fields.length})...`
            ...
            switch (event.t) {
              ...
              case 'builder_partial': {
                ...
                accumulatedRef.current = {
                  ...
                  ...(partial.tracker !== undefined ? { tracker: partial.tracker } : {}),
