...
export function useAnalystChat(options: UseAnalystChatOptions = {}) {
  ...
  const { object, submit, isLoading, error } = useObject({
    ...
    onError: (err: Error) => {
      ...
      const cid = conversationIdRef.current;
      if (cid) {
        persistMessage(cid, { role: "ASSISTANT", content: errorContent }).catch(
          (e) => console.error("Failed to persist analyst message:", e),
        );
      }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (trackerId && onConversationCreate) {
      try {
        ...
        console.error(
          "Failed to create analyst conversation via callback:",
          err,
        );
        skipPersist = true;
      }
