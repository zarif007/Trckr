...
function buildXYConfig(
  spec: Extract<AnalysisChartSpec, { type: "bar" | "line" | "area" }>,
): ChartConfig {
...
export function AnalysisBlockChart({ spec, data }: Props) {
  ...
  if (spec.type === "gantt") {
    ...
    const chartHeight = Math.min(
      520,
      Math.max(220, ganttData.length * 36 + 72),
    );
          ...
          >
            ...
            <ChartTooltip
              content={({ active, payload }) => {
                ...
                return (
                  <div className="rounded-sm border border-border bg-popover px-2.5 py-1.5 text-xs ">
