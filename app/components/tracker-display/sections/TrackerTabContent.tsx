...
function areTabContentPropsEqual(
  prev: TrackerTabContentProps,
  next: TrackerTabContentProps,
): boolean {
  if (
    prev.tab !== next.tab ||
...
export const TrackerTabContent = memo(function TrackerTabContent({
  ...
  const tabSections = useMemo(
    () =>
      sections
        .filter(
          (section) => section.tabId === tab.id && !section.config?.isHidden,
        )
  ...
  if (canEditLayout) {
    return (
      <TabsContent
        key={tab.id}
        value={tab.id}
        className={`${TAB_CONTENT_ROOT} data-[state=inactive]:hidden`}
        forceMount
      >
        <BlockEditor
          tab={tab}
