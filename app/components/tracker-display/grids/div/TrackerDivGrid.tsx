...
function TrackerDivGridInner({
  ...
  const { remove, move, applySchemaChange } = useLayoutActions(
    grid.id,
    schema,
    onSchemaChange,
  );
  ...
  const addOptionConfigByFieldId = useMemo(() => {
    ...
    optionFieldIds.forEach((fieldId) => {
      ...
      const canLocal = Boolean(onAddEntryToGrid && !sourceId);
      const canForeign = Boolean(sourceId && onAddForeign);
      if (!canLocal && !canForeign) return;
      ...
      map.set(fieldId, {
        ...
        onAddOption,
        optionsGridId,
        isMultiselect: field.dataType === "multiselect",
      });
    });
    return map;
  }, [
  ...
  const handleDragMove = useCallback(
    (event: DragMoveEvent) => {
      ...
      const placement =
        zone?.placement ??
        getDropPlacementByPointer(event.over?.rect, pointer, prevPlacement);
      if (!placement) {
        setDropIndicator((prev) => (prev ? null : prev));
        return;
      }
      ...
      const next = {
        overId: fieldSortableId(grid.id, targetFieldId),
        placement,
      };
  ...
  const handleFieldDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (!overId || active.id === overId || !applySchemaChange || !schema)
        return;
      const activeParsed = parseFieldId(String(active.id));
      const zone = parseDropZoneId(overId);
      const overParsed = zone
        ? { gridId: zone.gridId, fieldId: zone.fieldId }
        : parseFieldId(overId);
      if (
        !activeParsed ||
        !overParsed ||
      ...
      const currentNodes = (schema.layoutNodes ?? []).filter(
        (n) => n.gridId === grid.id,
      );
      const otherNodes = (schema.layoutNodes ?? []).filter(
        (n) => n.gridId !== grid.id,
      );
      const existingByField = new Map(
        currentNodes.map((n) => [n.fieldId, n] as const),
      );
      const rows = buildRowsFromNodes(currentNodes, DIV_GRID_MAX_COLS);
      const activeLoc = findRowIndex(rows, activeParsed.fieldId);
      if (!activeLoc) return;

      const [activeNode] = rows[activeLoc.rowIndex].splice(
        activeLoc.colIndex,
        1,
      );
      ...
      const verticalFallback: DropPlacement =
        pointer && overRect && pointer.y < overRect.top + overRect.height / 2
          ? "above"
          : "below";
      ...
      if (placement === "left" || placement === "right") {
        ...
        if (targetRow.length >= DIV_GRID_MAX_COLS) {
          const insertRowIndex =
            verticalFallback === "above"
              ? overLoc.rowIndex
              : overLoc.rowIndex + 1;
          rows.splice(insertRowIndex, 0, [activeNode]);
        } else {
          const insertIndex =
            placement === "left" ? overLoc.colIndex : overLoc.colIndex + 1;
          targetRow.splice(insertIndex, 0, activeNode);
        }
      } else {
        const insertRowIndex =
          placement === "above" ? overLoc.rowIndex : overLoc.rowIndex + 1;
        rows.splice(insertRowIndex, 0, [activeNode]);
      }
  ...
  const addOptionFieldMetadata: FieldMetadata = useMemo(() => {
    ...
    addOptionContext?.optionsGridId,
    validations,
  ]);
  ...
  const initialOptionValues = useMemo(() => {
    ...
    addOptionContext?.optionsGridFields?.forEach((f) => {
      initial[f.id] =
        f.type === "number" ? "" : f.type === "boolean" ? false : "";
    });
    return initial;
  }, [addOptionContext?.optionsGridFields]);
  ...
  const [dirtyFieldIds, setDirtyFieldIds] = useState<Set<string>>(
    () => new Set(),
  );
  ...
  const calculatedFieldIds = useMemo(() => {
    ...
    for (const path of Object.keys(calculations)) {
      ...
      if (ruleGridId === grid.id && fieldId) set.add(fieldId);
    }
    return set;
  ...
  const applyAddOption = useCallback(
    (values: Record<string, unknown>) => {
      ...
      if (addOptionContext.isMultiselect) {
        const currentVal =
          draftRow[addOptionContext.fieldId] ?? data[addOptionContext.fieldId];
  ...
  const fieldsContainer = (
    <div className="flex flex-col gap-2.5 min-w-0">
      {rowKeys.map((rowKey) => {
        ...
        const dropTargetFieldId = dropIndicator
          ? parseFieldId(dropIndicator.overId)?.fieldId
          : null;
        const isDropRow =
          dropTargetFieldId != null &&
          rowKeyByFieldId.get(dropTargetFieldId) === rowKey;
        const gridCols =
          GRID_COLS_CLASS[
            Math.min(Math.max(nodesInRow.length, 1), DIV_GRID_MAX_COLS)
          ] ?? "grid-cols-1";
        return (
          <div
            key={rowKey}
            className={`grid ${gridCols} gap-2.5 min-w-0 transition-[box-shadow,border-color] duration-150 ${isDropRow ? "ring-1 ring-primary/20 rounded-sm p-1 -m-1" : ""}`}
          >
            {nodesInRow.map((node) => {
              ...
              const indicator =
                canEditLayout &&
                activeDragId &&
                dropIndicator?.overId === fieldSortableId(grid.id, node.fieldId)
  ...
  return (
    <div className="space-y-2 min-w-0">
      ...
      {addOptionContext && addOptionContext.optionsGridFields.length > 0 && (
        <EntryFormDialog
          open={addOptionOpen}
          onOpenChange={(open) => {
