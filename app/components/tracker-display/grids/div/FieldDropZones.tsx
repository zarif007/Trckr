...
const FieldDropZones = memo(function FieldDropZones({
  ...
  const leftZone = useDroppable({
    id: fieldDropZoneId(gridId, fieldId, "left"),
  });
  const rightZone = useDroppable({
    id: fieldDropZoneId(gridId, fieldId, "right"),
  });
  const topZone = useDroppable({
    id: fieldDropZoneId(gridId, fieldId, "above"),
  });
  const bottomZone = useDroppable({
    id: fieldDropZoneId(gridId, fieldId, "below"),
  });
  ...
  return (
    <>
      ...
      <div
        ...
        className="pointer-events-none absolute left-0 top-[20%] bottom-[20%] w-1/2"
      />
      <div
        ref={rightZone.setNodeRef}
        className="pointer-events-none absolute right-0 top-[20%] bottom-[20%] w-1/2"
      />
    </>
  );
});
