...
function toTrackerOptions(
  ...
  return raw.map((o) =>
    typeof o === "string"
      ? { label: o, value: o }
      : { label: o.label, value: o.id, id: o.id },
  );
}
  ...
  return (
    ...
    >
              ...
              <div className="flex flex-col gap-0.5">
                {onDeleteRow != null && (
                  <Button
                    variant="ghost"
                    size="sm"
                    className="h-8 justify-start gap-2 rounded-sm px-2 text-xs text-destructive hover:bg-destructive/10 hover:text-destructive font-normal"
                    onClick={(e) => {
                      e.stopPropagation();
                      onDeleteRow();
                    }}
      ...
      <div
        className={cn(
          "min-w-0",
          (showEditButton || showSettingsButton) && "pr-16",
        )}
      >
        {cardFields.map((field) => {
          ...
          return (
            <div key={field.id} className="mb-2.5 last:mb-0">
              <p
                className={`${labelFontSize} text-muted-foreground font-medium mb-0.5`}
              >
                {field.label}
              </p>
              <div className={`${valueFontSize} ${valueTextColor} break-words`}>
                <TrackerCell
                  ...
                  type={field.dataType}
                  options={options}
                  config={fieldMetadata?.[field.id]?.config}
                />
              </div>
            </div>
          );
