...
export function buildOptionsGridOptions(
  grids: TrackerGrid[],
): FieldPathOption[] {
  return (grids ?? [])
...
export function buildAllGridsPickerOptions(
  grids: TrackerGrid[],
): FieldPathOption[] {
  return (grids ?? [])
    .map((g) => ({ value: g.id, label: g.name ?? g.id }))
...
export function buildGridFieldMap(
  layoutNodes: TrackerLayoutNode[],
...
export function normalizeMappings(
  raw: FieldMapping[] | undefined | null,
): FieldMapping[] {
...
export function validateBindingDraft(
  ...
  const optionsSideMap =
    ...
    context.sourceGridFieldMap.size > 0
      ? context.sourceGridFieldMap
  ...
  const selectGridId = key
    ? (parsePath(key).gridId ?? key.split(".")[0])
    : undefined;
  const optionsFieldIds = optionsGrid
    ? optionsSideMap.get(optionsGrid)
    : undefined;
  const mainFieldIds = selectGridId
    ? context.gridFieldMap.get(selectGridId)
    : undefined;
    ...
    for (const mapping of mappings) {
      ...
      if (optionsFieldIds) {
        if (fromParsed.gridId && fromParsed.gridId !== optionsGrid) {
          errors.fieldMappings =
            "Mapping sources must be from the master data grid.";
          break;
        }
      ...
      if (mainFieldIds) {
        if (
          toParsed.gridId &&
          selectGridId &&
          toParsed.gridId !== selectGridId
        ) {
          errors.fieldMappings =
            "Mapping targets must be from the select field grid.";
          break;
        }
...
export function suggestFieldMappings(
  ...
  const existingSet = new Set(
    existingMappings.map((m) => `${m.from}\t${m.to}`),
  );
