...
export function useForeignBindingSources(
  ...
  >({})
  const [foreignPersistMetaBySchemaId, setForeignPersistMetaBySchemaId] = useState<
    Record<string, ForeignDataPersistMeta>
  >({})
  ...
  const [foreignPersistError, setForeignPersistError] = useState<ForeignPersistError | null>(null)
  ...
  const refetchForeignGridData = useCallback(async (sourceSchemaId: string) => {
    ...
    if (!row) return
    setForeignGridDataBySchemaId((prev) => ({ ...prev, [sourceSchemaId]: row.gridData }))
    setForeignPersistMetaBySchemaId((prev) => {
      ...
      return {
        ...
        [sourceSchemaId]: {
          ...cur,
          dataSnapshotId:
            typeof row.dataSnapshotId === 'string' ? row.dataSnapshotId : cur.dataSnapshotId,
  ...
  const flushPersistForSource = useCallback(
      ...
      for (;;) {
        ...
        try {
          ...
          if (result.kind === 'saved') {
            ...
            console.error('Failed to persist options on source tracker', sourceSchemaId, result.message)
    ...
    [applyPersistSuccess, beginSave, clearDebounce, endSave, refetchForeignGridData]
  )
  ...
  useEffect(() => {
    const ids = bindingSourceIdsKey === '' ? [] : bindingSourceIdsKey.split('\0')
    let cancelled = false
    ...
    ;(async () => {
      const bundles = await Promise.all(ids.map((id) => loadForeignBindingSource(id)))
      if (cancelled) {
        setForeignSourcesLoading(false)
        return
      }
