  ...
  const handleRemoveTab = useCallback(
    (tabId: string) => {
      ...
      const nextSections = (sections ?? []).filter(
        (section) => section.tabId !== tabId,
      );
      const nextGrids = (grids ?? []).filter(
        (grid) => !sectionIdsInTab.has(grid.sectionId),
      );
      const removedGridIds = new Set(
        (grids ?? [])
          .filter((grid) => sectionIdsInTab.has(grid.sectionId))
          .map((grid) => grid.id),
      );
      const nextLayoutNodes = (layoutNodes ?? []).filter(
        (node) => !removedGridIds.has(node.gridId),
      );
