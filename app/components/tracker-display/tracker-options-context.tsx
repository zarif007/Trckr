...
import type {
  DynamicOptionsDefinitions,
  ForeignBindingSourceSchema,
} from "@/lib/dynamic-options";

const TrackerOptionsContext = createContext<TrackerContextForOptions | null>(
  null,
);
...
export function TrackerOptionsProvider({
  ...
  const value = useMemo<TrackerContextForOptions>(
    () => ({
      ...
      ...(foreignGridDataBySchemaId &&
      Object.keys(foreignGridDataBySchemaId).length > 0
        ? { foreignGridDataBySchemaId }
        : {}),
      ...(foreignSchemaBySchemaId &&
      Object.keys(foreignSchemaBySchemaId).length > 0
        ? { foreignSchemaBySchemaId }
        : {}),
