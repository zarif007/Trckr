...
export function useSectionGridActions(
  ...
  const apply = useCallback(
    (
      patch: Partial<
        Pick<TrackerDisplayProps, "sections" | "grids" | "layoutNodes">
      >,
    ) => {
  ...
  const removeSection = useCallback(
    (sectionId: string) => {
      ...
      const grids = (schema.grids ?? []).filter(
  ...
  const moveSection = useCallback(
    (sectionId: string, direction: "up" | "down") => {
      ...
      const tabSections = sections
        .filter(
          (s) => sections.find((x) => x.id === sectionId)?.tabId === s.tabId,
        )
  ...
  const moveGrid = useCallback(
    (gridId: string, direction: "up" | "down") => {
      ...
      const sectionGrids = grids
        .filter(
          (g) => grids.find((x) => x.id === gridId)?.sectionId === g.sectionId,
        )
        .sort((a, b) => a.placeId - b.placeId);
