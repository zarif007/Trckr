...
export function ExprFlowBuilder({
  ...
  const initialGraph = useMemo(
    () => exprToGraph(normalizeExprNode(expr)),
    [expr],
  );
  ...
  const nodeCountExcludingResult = nodes.filter(
    (n) => n.type !== "result",
  ).length;
  ...
  const deleteSelection = useCallback(() => {
    ...
    const nodeIdsToDelete = new Set(
      selectedNodeIds.filter(
        (id) => nodes.find((node) => node.id === id)?.type !== "result",
      ),
    );
    setEdges((prev) =>
      prev.filter(
  ...
  const enrichNode = useCallback(
    (node: FlowNode): FlowNode => ({
      ...
      data: {
        ...
        availableFields,
        ...(node.type === "result" ? { resultFieldId, resultFieldLabel } : {}),
      },
    }),
    [
      availableFields,
  ...
  const makeDragStart = useCallback(
    (payload: object) => (event: React.DragEvent) => {
