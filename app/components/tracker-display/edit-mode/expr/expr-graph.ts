...
function parseConstValue(value: string | undefined): {
  ok: boolean;
  value?: unknown;
} {
...
export function compileExprFromGraph(
  ...
  if (rootNode?.type === "accumulator") {
    ...
    const accSourceNode = accSource
      ? (nodeMap.get(accSource) as ExprFlowNode | undefined)
      : undefined;
    if (
      !accSource ||
      !accSourceNode ||
      accSourceNode.type !== "field" ||
      !accSourceNode.data?.fieldId
    ) {
  ...
  const build = (nodeId: string): ExprNode | null => {
    ...
    if (node.type === "accumulator") {
      const sourceSource = getSingleSource(
        nodeIncoming,
      ...
      const endIndex = end != null && Number.isInteger(end) ? end : undefined;
    ...
    if (node.type === "logic") {
      ...
      if (logicOp === "if") {
        ...
        if (!condSource || !thenSource || !elseSource) {
          visiting.delete(nodeId);
          visited.add(nodeId);
          return null;
        }
    ...
    if (node.type === "math") {
      ...
      if (mathOp === "clamp") {
        ...
        if (!valueSource || !minSource || !maxSource) {
          visiting.delete(nodeId);
          visited.add(nodeId);
          return null;
        }
    ...
    if (node.type === "string") {
      ...
      if (stringOp === "slice") {
        ...
        if (!valueSource || !startSource || !endSource) {
          visiting.delete(nodeId);
          visited.add(nodeId);
          return null;
        }
  ...
  if (!expr)
    return { error: "Graph is missing required inputs or has a cycle." };
...
export function exprToGraph(expr: ExprNode): {
  nodes: ExprFlowNode[];
  edges: Edge[];
} {
  nodeIdCounter = 0;
  ...
  const build = (node: ExprNode, depth: number): string => {
      ...
      edges.push(
        {
          ...
          source: leftId,
          target: id,
          targetHandle: INPUT_HANDLES[0],
        },
        {
          id: `${rightId}-${id}-b`,
          source: rightId,
          target: id,
          targetHandle: INPUT_HANDLES[1],
        },
      );
      return id;
    }
    ...
    if (
      ["sub", "div", "eq", "neq", "gt", "gte", "lt", "lte"].includes(node.op)
    ) {
      nodes.push({
        id,
        type: "op",
      ...
      edges.push(
        {
          ...
          source: condId,
          target: id,
          targetHandle: IF_HANDLES.cond,
        },
        {
          id: `${thenId}-${id}-then`,
          source: thenId,
          target: id,
          targetHandle: IF_HANDLES.then,
        },
        {
          id: `${elseId}-${id}-else`,
          source: elseId,
          target: id,
          targetHandle: IF_HANDLES.else,
        },
      );
      return id;
    }
    ...
    if (node.op === "and" || node.op === "or") {
      const args = (node as Extract<ExprNode, { op: "and" | "or" }>).args ?? [
        PLACEHOLDER,
        PLACEHOLDER,
      ];
      ...
      nodes.push({
        ...
        type: "logic",
        position,
        data: { logicOp: node.op, inputCount: count },
      });
      for (let i = 0; i < count; i++) {
        ...
        edges.push({
          ...
          source: argId,
          target: id,
          targetHandle: variadicHandleId(i),
        });
      }
      return id;
    }
    ...
    if (["abs", "round", "floor", "ceil"].includes(node.op)) {
      const unaryNode = node as Extract<
        ExprNode,
        { op: "abs" | "round" | "floor" | "ceil" }
      >;
      nodes.push({
        id,
        type: "math",
        position,
        data: { mathOp: node.op as MathOp },
      });
    ...
    if (node.op === "min" || node.op === "max") {
      ...
      nodes.push({
        ...
        type: "math",
        position,
        data: { mathOp: node.op, inputCount: count },
      });
      for (let i = 0; i < count; i++) {
        const argId = build(args[i] ?? PLACEHOLDER, depth + 1);
        edges.push({
      ...
      edges.push(
        {
          ...
          source: valueId,
          target: id,
          targetHandle: CLAMP_HANDLES.value,
        },
        {
          id: `${minId}-${id}-min`,
          source: minId,
          target: id,
          targetHandle: CLAMP_HANDLES.min,
        },
        {
          id: `${maxId}-${id}-max`,
          source: maxId,
          target: id,
          targetHandle: CLAMP_HANDLES.max,
        },
      );
      return id;
    }
    ...
    if (["length", "trim", "toUpper", "toLower"].includes(node.op)) {
      const unaryNode = node as Extract<
        ExprNode,
        { op: "length" | "trim" | "toUpper" | "toLower" }
      >;
      nodes.push({
        id,
        type: "string",
        position,
        data: { stringOp: node.op as StringOp },
      });
      const argId = build(unaryNode.arg ?? PLACEHOLDER, depth + 1);
      edges.push({
        id: `${argId}-${id}-a`,
    ...
    if (node.op === "concat") {
      ...
      nodes.push({
        ...
        type: "string",
        position,
        data: { stringOp: "concat", inputCount: count },
      });
      for (let i = 0; i < count; i++) {
        const argId = build(args[i] ?? PLACEHOLDER, depth + 1);
        edges.push({
      ...
      edges.push(
        {
          ...
          source: valueId,
          target: id,
          targetHandle: SLICE_HANDLES.value,
        },
        {
          id: `${startId}-${id}-start`,
          source: startId,
          target: id,
          targetHandle: SLICE_HANDLES.start,
        },
        {
          id: `${endId}-${id}-end`,
          source: endId,
          target: id,
          targetHandle: SLICE_HANDLES.end,
        },
      );
      return id;
    }
    ...
    if (node.op === "regex") {
      nodes.push({
        ...
        type: "string",
        position,
        data: { stringOp: "regex", pattern: node.pattern, flags: node.flags },
      });
      const valueId = build(node.value ?? PLACEHOLDER, depth + 1);
      edges.push({
        id: `${valueId}-${id}-a`,
  ...
  const orientedNodes = nodes.map((node) => {
    ...
    return {
      ...node,
      position: { ...node.position, x: (maxDepth - depth) * 280 },
    };
  });
