...
export type NodeUpdater = (id: string, partial: Partial<ExprFlowNodeData>) => void
...
const NODE_BASE_CLASSES = 'overflow-hidden rounded-sm border border-border/50 bg-background transition-all duration-150 hover:'
...
const NODE_ICON_CLASSES = 'h-[22px] w-[22px] rounded-sm flex items-center justify-center flex-shrink-0'
...
function AccumulatorNode({ id, data }: { id: string; data: FlowNodeData }) {
 ...
 if (v === '') { data.onChange?.(id, { startIndex: undefined }); return }
 ...
 onChange={(e) => {
 const v = e.target.value
 if (v === '') { data.onChange?.(id, { endIndex: undefined }); return }
 const n = parseInt(v, 10)
 ...
 onChange={(e) => {
 const v = e.target.value
 if (v === '') { data.onChange?.(id, { initialValue: undefined }); return }
 const n = Number(v)
...
function LogicNode({ id, data }: { id: string; data: FlowNodeData }) {
 ...
 <span><strong className="text-foreground/80">Condition</strong> — connect a comparison (Equal, &gt;, &lt;, …)</span>
 </div>
 <div className="flex items-center gap-1.5">
 <div className="h-1.5 w-1.5 rounded-full bg-indigo-400 shrink-0" />
 <span><strong className="text-foreground/80">Then</strong> — value when true</span>
 </div>
 <div className="flex items-center gap-1.5">
 <div className="h-1.5 w-1.5 rounded-full bg-indigo-400 shrink-0" />
 <span><strong className="text-foreground/80">Else</strong> — value when false</span>
 </div>
 </div>
 )}
 {isVariadic && (
 ...
 {isNot && (
 <Handle type="target" position={Position.Left} id="a" style={{ top: HANDLE_OFFSET_START }} className={cn(HANDLE_CLASSES, '!border-indigo-500')} />
 )}
 {isIf && (
 <>
 <Handle type="target" position={Position.Left} id={FLOW_CONSTANTS.IF_HANDLES.cond} style={{ top: HANDLE_OFFSET_START }} className={cn(HANDLE_CLASSES, '!border-indigo-500')} />
 <Handle type="target" position={Position.Left} id={FLOW_CONSTANTS.IF_HANDLES.then} style={{ top: HANDLE_OFFSET_START + HANDLE_OFFSET_STEP }} className={cn(HANDLE_CLASSES, '!border-indigo-500')} />
 <Handle type="target" position={Position.Left} id={FLOW_CONSTANTS.IF_HANDLES.else} style={{ top: HANDLE_OFFSET_START + HANDLE_OFFSET_STEP * 2 }} className={cn(HANDLE_CLASSES, '!border-indigo-500')} />
 </>
 )}
 {isVariadic && Array.from({ length: inputCount }, (_, i) => (
 <Handle
 ...
 <Handle type="source" position={Position.Right} id="out" className={cn(HANDLE_CLASSES, '!border-indigo-500')} />
 </div>
 )
}
...
function MathNode({ id, data }: { id: string; data: FlowNodeData }) {
 ...
 <div className="flex items-center gap-1.5"><div className="h-1.5 w-1.5 rounded-full bg-orange-400" /><span>{mathOp === 'pow' ? 'Base (A)' : 'Dividend (A)'}</span></div>
 <div className="flex items-center gap-1.5"><div className="h-1.5 w-1.5 rounded-full bg-orange-400" /><span>{mathOp === 'pow' ? 'Exponent (B)' : 'Divisor (B)'}</span></div>
 </div>
 )}
 {shape === 'ternary' && MATH_TERNARY_LABELS[mathOp] && (
 <div className="space-y-1 text-[10px] text-muted-foreground">
 {MATH_TERNARY_LABELS[mathOp].map((lbl) => (
 <div key={lbl} className="flex items-center gap-1.5"><div className="h-1.5 w-1.5 rounded-full bg-orange-400" /><span>{lbl}</span></div>
 ))}
 </div>
 )}
 {shape === 'variadic' && (
 ...
 {shape === 'unary' && <Handle type="target" position={Position.Left} id="a" style={{ top: HANDLE_OFFSET_START }} className={cn(HANDLE_CLASSES, '!border-orange-500')} />}
 {shape === 'binary' && (
 <>
 <Handle type="target" position={Position.Left} id={FLOW_CONSTANTS.INPUT_HANDLES[0]} style={{ top: HANDLE_OFFSET_START }} className={cn(HANDLE_CLASSES, '!border-orange-500')} />
 <Handle type="target" position={Position.Left} id={FLOW_CONSTANTS.INPUT_HANDLES[1]} style={{ top: HANDLE_OFFSET_START + HANDLE_OFFSET_STEP }} className={cn(HANDLE_CLASSES, '!border-orange-500')} />
 </>
 )}
 {shape === 'ternary' && (
 <>
 <Handle type="target" position={Position.Left} id={FLOW_CONSTANTS.CLAMP_HANDLES.value} style={{ top: HANDLE_OFFSET_START }} className={cn(HANDLE_CLASSES, '!border-orange-500')} />
 <Handle type="target" position={Position.Left} id={FLOW_CONSTANTS.CLAMP_HANDLES.min} style={{ top: HANDLE_OFFSET_START + HANDLE_OFFSET_STEP }} className={cn(HANDLE_CLASSES, '!border-orange-500')} />
 <Handle type="target" position={Position.Left} id={FLOW_CONSTANTS.CLAMP_HANDLES.max} style={{ top: HANDLE_OFFSET_START + HANDLE_OFFSET_STEP * 2 }} className={cn(HANDLE_CLASSES, '!border-orange-500')} />
 </>
 )}
 {shape === 'variadic' && Array.from({ length: inputCount }, (_, i) => (
 <Handle
 key={variadicHandleId(i)}
 type="target"
 position={Position.Left}
 ...
 <Handle type="source" position={Position.Right} id="out" className={cn(HANDLE_CLASSES, '!border-orange-500')} />
 </div>
 )
}
...
function StringNode({ id, data }: { id: string; data: FlowNodeData }) {
 ...
 {shape === 'binary' && (
 <div className="space-y-1 text-[10px] text-muted-foreground">
 <div className="flex items-center gap-1.5"><div className="h-1.5 w-1.5 rounded-full bg-rose-400" /><span>Haystack (A)</span></div>
 <div className="flex items-center gap-1.5"><div className="h-1.5 w-1.5 rounded-full bg-rose-400" /><span>Needle (B)</span></div>
 </div>
 )}
 {shape === 'ternary' && (
 <div className="space-y-1 text-[10px] text-muted-foreground">
 <div className="flex items-center gap-1.5"><div className="h-1.5 w-1.5 rounded-full bg-rose-400" /><span>String (top)</span></div>
 <div className="flex items-center gap-1.5"><div className="h-1.5 w-1.5 rounded-full bg-rose-400" /><span>Start index</span></div>
 <div className="flex items-center gap-1.5"><div className="h-1.5 w-1.5 rounded-full bg-rose-400" /><span>End index</span></div>
 </div>
 )}
 {shape === 'variadic' && (
 <div className="flex items-center gap-1.5">
 ...
 <div className="h-1.5 w-1.5 rounded-full bg-rose-400" /><span>String input (left)</span>
 </div>
 <Input
 value={data.pattern ?? ''}
 ...
 {shape === 'unary' && <Handle type="target" position={Position.Left} id="a" style={{ top: HANDLE_OFFSET_START }} className={cn(HANDLE_CLASSES, '!border-rose-500')} />}
 {shape === 'binary' && (
 <>
 <Handle type="target" position={Position.Left} id={FLOW_CONSTANTS.INPUT_HANDLES[0]} style={{ top: HANDLE_OFFSET_START }} className={cn(HANDLE_CLASSES, '!border-rose-500')} />
 <Handle type="target" position={Position.Left} id={FLOW_CONSTANTS.INPUT_HANDLES[1]} style={{ top: HANDLE_OFFSET_START + HANDLE_OFFSET_STEP }} className={cn(HANDLE_CLASSES, '!border-rose-500')} />
 </>
 )}
 {shape === 'ternary' && (
 <>
 <Handle type="target" position={Position.Left} id={FLOW_CONSTANTS.SLICE_HANDLES.value} style={{ top: HANDLE_OFFSET_START }} className={cn(HANDLE_CLASSES, '!border-rose-500')} />
 <Handle type="target" position={Position.Left} id={FLOW_CONSTANTS.SLICE_HANDLES.start} style={{ top: HANDLE_OFFSET_START + HANDLE_OFFSET_STEP }} className={cn(HANDLE_CLASSES, '!border-rose-500')} />
 <Handle type="target" position={Position.Left} id={FLOW_CONSTANTS.SLICE_HANDLES.end} style={{ top: HANDLE_OFFSET_START + HANDLE_OFFSET_STEP * 2 }} className={cn(HANDLE_CLASSES, '!border-rose-500')} />
 </>
 )}
 {shape === 'variadic' && Array.from({ length: inputCount }, (_, i) => (
 <Handle
 ...
 {shape === 'regex' && <Handle type="target" position={Position.Left} id="a" style={{ top: HANDLE_OFFSET_START }} className={cn(HANDLE_CLASSES, '!border-rose-500')} />}
 <Handle type="source" position={Position.Right} id="out" className={cn(HANDLE_CLASSES, '!border-rose-500')} />
 </div>
 )
}
