...
export interface CalculationsTabProps {
  ...
  setCalculationRule: React.Dispatch<
    React.SetStateAction<FieldCalculationRule | null>
  >;
...
export function CalculationsTab({
    ...
    return (
      <RuleEmptyState
        ...
        action={{
          ...
          onClick: () =>
            setCalculationRule({
              expr: defaultCalculationExpr,
            } as FieldCalculationRule),
        }}
      />
    );
  }
  ...
  const badges = (
    <>
      <Badge
        variant="outline"
        className="text-[10px] h-4 px-1.5 shrink-0 bg-primary/10 border-primary/20 text-primary"
      >
        Calculation
      </Badge>
