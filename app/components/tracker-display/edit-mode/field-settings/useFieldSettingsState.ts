...
export function useFieldSettingsState({
  ...
  const { projectId: ctxProjectId, trackerSchemaId: ctxTrackerSchemaId } =
    useEditMode();
  const field = useMemo(() => {
  ...
  const [dateFormat, setDateFormat] = useState<"iso" | "us" | "eu" | "long">(
    "long",
  );
  ...
  const [calculationRule, setCalculationRule] =
    useState<FieldCalculationRule | null>(null);
  ...
  const [siblingTrackers, setSiblingTrackers] = useState<
    Array<{ id: string; name: string | null }>
  >([]);
  const [siblingsLoading, setSiblingsLoading] = useState(false);
  const [sourceSchema, setSourceSchema] = useState<TrackerDisplayProps | null>(
    null,
  );
  const [sourceSchemaLoading, setSourceSchemaLoading] = useState(false);
  const [dynamicOptionsDraft, setDynamicOptionsDraft] =
    useState<DynamicOptionsDefinitions>({});
  ...
  const [dynamicConfigError, setDynamicConfigError] = useState<string | null>(
    null,
  );
  ...
  const isDynamicField =
    dataType === "dynamic_select" || dataType === "dynamic_multiselect";
    ...
    (async () => {
      ...
      try {
        const res = await fetch(
          `/api/projects/${encodeURIComponent(ctxProjectId)}/trackers`,
        );
        if (!res.ok) {
        ...
        if (!cancelled)
          setSiblingTrackers(Array.isArray(data.items) ? data.items : []);
      } catch {
        if (!cancelled) setSiblingTrackers([]);
      } finally {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled)
          setSourceSchema((data.schema ?? null) as TrackerDisplayProps | null);
      } catch {
        if (!cancelled) setSourceSchema(null);
      } finally {
  ...
  useEffect(() => {
    ...
    setPrefix(
      typeof field.config?.prefix === "string" ? field.config.prefix : "",
    );
    ...
    setMinLength(
      field.config?.minLength != null ? String(field.config.minLength) : "",
    );
    setMaxLength(
      field.config?.maxLength != null ? String(field.config.maxLength) : "",
    );
    setNumberDecimalPlaces(
      field.config?.numberDecimalPlaces != null
        ? String(field.config.numberDecimalPlaces)
        : "",
    );
    setNumberStep(
      field.config?.numberStep != null ? String(field.config.numberStep) : "",
    );
    ...
    setDateFormat(
      savedDateFormat === "iso" ||
        savedDateFormat === "us" ||
        savedDateFormat === "eu" ||
        savedDateFormat === "long"
        ? savedDateFormat
        : "long",
    );
    setRatingMax(
      field.config?.ratingMax != null ? String(field.config.ratingMax) : "",
    );
    setRatingAllowHalf(Boolean(field.config?.ratingAllowHalf));
    setPersonAllowMultiple(Boolean(field.config?.personAllowMultiple));
    setFilesMaxCount(
      field.config?.filesMaxCount != null
        ? String(field.config.filesMaxCount)
        : "",
    );
    setFilesMaxSizeMb(
      field.config?.filesMaxSizeMb != null
        ? String(field.config.filesMaxSizeMb)
        : "",
    );
    setStatusOptionsText(
      Array.isArray(field.config?.statusOptions)
        ? field.config.statusOptions.join("\n")
        : "",
    );

    const validationKey = gridId ? `${gridId}.${field.id}` : "";
    const nextRules = (schema?.validations?.[validationKey] ?? []).map(
      ensureRuleDefaults,
    );
    setRules(nextRules);
    const nextCalculation = validationKey
      ? (schema?.calculations?.[validationKey] ?? null)
      : null;
    ...
    setCalculationRule(
      nextCalculation && nextCalculation.expr ? nextCalculation : null,
    );
    const isBindableField =
      field.dataType === "options" || field.dataType === "multiselect";
    if (isBindableField && bindingKey) {
      const existingBinding = schema?.bindings?.[bindingKey] as
        | TrackerBindingEntry
        | undefined;
      if (existingBinding) {
        ...
        setBindingDraft({
          ...
          optionsSourceSchemaId:
            existingBinding.optionsSourceSchemaId?.trim() || undefined,
    ...
    setDynamicOptionsDraft(
      (schema?.dynamicOptions as DynamicOptionsDefinitions | undefined) ?? {},
    );
    setDynamicFunctionId(
      (field.config as { dynamicOptionsFunction?: string } | undefined)
    ...
    setDynamicOptionsArgsText(
      dynamicArgs ? JSON.stringify(dynamicArgs, null, 2) : "",
    );
    const dynamicTtl = (
      field.config as { dynamicOptionsCacheTtlSeconds?: number } | undefined
    )?.dynamicOptionsCacheTtlSeconds;
    setDynamicCacheTtlText(
      typeof dynamicTtl === "number" && Number.isFinite(dynamicTtl)
        ? String(dynamicTtl)
        : "",
    );
    setDynamicConfigError(null);
  ...
  const bindingValidation = useMemo(() => {
    if (!bindingEnabled || !bindingDraft)
  ...
  const validationKey = useMemo(
    () => (gridId && field ? `${gridId}.${field.id}` : ""),
    [gridId, field],
  );
  const hasCalculation = Boolean(
    validationKey && schema?.calculations?.[validationKey]?.expr,
  );
  ...
  const updateRule = useCallback(
    (index: number, nextRule: FieldValidationRule) => {
      setRules((prev) =>
        prev.map((r, i) => (i === index ? ensureRuleDefaults(nextRule) : r)),
      );
    },
    [],
  );
    ...
    (pick: {
      ...
      setBindingDraft((prev) => {
        ...
        if (lf && bindingKey) {
          next = {
            ...next,
            fieldMappings: ensureValueMapping(
              normalizeMappings(next.fieldMappings),
              lf,
              bindingKey,
            ),
          };
  ...
  const applyAutoMappings = useCallback(() => {
    ...
    setBindingDraftValue({
      ...bindingDraft,
      fieldMappings: [...existing, ...suggestions],
    });
  }, [
    bindingDraft,
    bindingKey,
  ...
  const handleRuleTypeChange = useCallback(
    (index: number, nextType: FieldValidationRule["type"]) => {
      setRules((prev) => {
        const next = [...prev];
  ...
  const handleSave = useCallback(() => {
    ...
    const normalizedStatusOptions = statusOptionsText
      ...
      .map((line) => line.trim())
      .filter(Boolean);
    nextConfig.statusOptions =
      normalizedStatusOptions.length > 0 ? normalizedStatusOptions : undefined;
    ...
    if (isDynamicField) {
      ...
      const parsedTtl =
        dynamicCacheTtlText.trim() === ""
          ? undefined
          : Number(dynamicCacheTtlText);
    ...
    onSchemaChange({
      ...
      fields: nextFields,
      validations:
        Object.keys(nextValidations).length > 0 ? nextValidations : undefined,
      calculations:
        Object.keys(nextCalculations).length > 0 ? nextCalculations : undefined,
      bindings: nextBindings,
      fieldRulesV2:
        Object.keys(nextFieldRulesV2Map).length > 0
          ? nextFieldRulesV2Map
          : undefined,
  ...
  const resolveAutoPopulateFromPathLabelFn = useCallback(
    (fromPath: string) => {
      ...
      const trackerTitle =
        siblingTrackers.find((t) => t.id === sid)?.name?.trim() ||
        "Linked tracker";
