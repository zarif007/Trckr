...
import { buildGridFieldMap, buildAllGridsPickerOptions, resolvePathLabel } from '../../bindings/bindings-utils'
...
export function BindingSourceCascade({
 ...
 }, [bindingDraft, siblingTrackers, currentTrackerName, pickerSchema, localSchema])

 const foreignLoading = Boolean(draftSourceId) && (sourceSchemaLoading || !pickerSchema)

 const selectThisTracker = () => {
 onPick({ optionsSourceSchemaId: undefined, optionsGrid: '', labelField: '' })
 setStep('grid')
 }
 ...
 {currentTrackerName?.trim() || currentTrackerSchemaId || 'Current schema'}
 </div>
 </button>
 {!projectId ? (
 ...
 <div className="px-3 py-2 text-xs text-muted-foreground">Loading trackers…</div>
 ) : (
 siblingTrackers
 ...
 {foreignLoading ? (
 <div className="px-3 py-2 text-xs text-muted-foreground">
 {sourceSchemaLoading ? 'Loading schema…' : 'Could not load source tracker schema.'}
 </div>
 ) : gridPickerOptions.length === 0 ? (
 <div className="px-3 py-2 text-xs text-muted-foreground">No grids in this tracker.</div>
 ) : (
 ...
 if (!ids || ids.size === 0) {
 return (
 <div className="px-3 py-2 text-xs text-muted-foreground">No fields in this grid.</div>
 )
 }
