...
export function createNewFieldId(
  label: string,
...
export function getNextRowCol(
  ...
  if (withPos.length === 0) {
    const order =
      nodes.length === 0 ? 0 : Math.max(...nodes.map((n) => n.order)) + 1;
...
export function getCreatableFieldTypesWithLabels(): FieldTypeOption[] {
  return FIELD_TYPE_ORDER.map((value) => ({
    ...
    group: Object.entries(FIELD_TYPE_GROUPS).find(([, types]) =>
