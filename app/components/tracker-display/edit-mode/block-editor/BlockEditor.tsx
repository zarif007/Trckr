...
import {
  getOrCreateSectionAndGridForField,
...
function parseBlockSortableId(sortableId: string): FlatBlock | null {
  ...
  if (sortableId.startsWith(GRID_PREFIX)) {
    return {
      type: "grid",
      id: sortableId.slice(GRID_PREFIX.length),
      sectionId: "",
    };
  }
  return null;
...
export function BlockEditor({
  ...
  const [insertInserterAfterBlockIndex, setInsertInserterAfterBlockIndex] =
      ...
      >
        <div className={`${SECTION_STACK_GAP} w-full min-w-0`}>
              ...
              >
                <SectionBar name={group.section.name}>
                  <InlineEditableName
                    ...
                    onChange={(name) =>
                      actions.renameSection(group.section.id, name)
                    }
                  />
                </SectionBar>
              </SortableBlockItem>
                      ...
                      >
                        <div className={GRID_BLOCK_INNER}>
                          <GridBlockHeader
                            ...
                            editable
                            onNameChange={(name) =>
                              actions.renameGrid(grid.id, name)
                            }
                          />
                          <GridBlockContent
                            tabId={tab.id}
          ...
          <div className="flex pt-2 pb-0 w-full">
            <div className="flex-1 min-w-0">
              <BlockCommandInput {...getCommandProps(flatBlocks.length)} />
            </div>
          </div>
