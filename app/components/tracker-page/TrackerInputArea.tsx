  ...
  return (
    <div className={isHero ? 'space-y-4' : 'space-y-3'}>
      <div className="relative group">
        <div
          className={`relative overflow-hidden transition-[border-color,box-shadow] duration-200 ${isHero ? 'bg-card rounded-md border border-border/60 shadow-sm' : 'bg-background rounded-md border border-border/50 shadow-none'} ${isFocused ? 'ring-1 ring-foreground/10 border-border/60 shadow-xs' : ''}`}
        >
          <div className={`flex items-end gap-2 ${isHero ? 'p-3 md:p-4' : 'p-2.5 pl-4'}`}>
            ...
            <Button
              ...
              className={`shrink-0 transition-all disabled:opacity-40 disabled:cursor-not-allowed ${isHero ? 'h-12 w-12 rounded-md' : 'h-9 w-9 rounded-md'} ${input.trim() && !isLoading
