...
export function TrackerBranchPanel({
  ...
  const mainBranch = branches.find(
    (b) => b.branchName === "main" && !b.isMerged,
  );
  const activeBranches = useMemo(
    () => branches.filter((b) => !b.isMerged),
    [branches],
  );
  ...
  const handleMergeToMain = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/trackers/${trackerId}/branches/${currentBranch.id}/merge`,
        {
          method: "POST",
        },
      );
  ...
  const diffData = useMemo<{
    ...
    const mainData =
      mainBranch.data && typeof mainBranch.data === "object"
        ? mainBranch.data
        : {};
    const currentData =
      currentBranch.data && typeof currentBranch.data === "object"
        ? currentBranch.data
        : {};
    return {
  ...
  return (
    <>
      <div
        className={cn(
          "absolute top-0 right-0 h-full z-30 flex flex-col bg-background border-l border-border/60 transition-all duration-200 ease-in-out",
          open
      ...
      >
        ...
        <div className="flex-shrink-0 flex items-center justify-between h-11 px-3 border-b border-border/50">
          ...
          <button
            type="button"
            onClick={onClose}
            className="h-6 w-6 rounded-sm flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors"
          >
        ...
        {/* Status messages */}
        {successMsg && (
          <div className="flex-shrink-0 mx-3 mt-2 flex items-center gap-1.5 px-2 py-1.5 rounded-sm bg-emerald-500/10 border border-emerald-500/20 text-[11px] text-emerald-600 dark:text-emerald-400">
        ...
        {error && (
          <button
            ...
            className="flex-shrink-0 mx-3 mt-2 flex items-center gap-1.5 px-2 py-1.5 rounded-sm bg-destructive/10 border border-destructive/20 text-[11px] text-destructive hover:bg-destructive/15 transition-colors"
          >
            ...
            {activeBranches.map((branch) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "w-full flex items-center gap-2.5 px-2.5 py-2 rounded-sm text-left text-xs transition-colors",
                    isActive
              ...
              {showMerged && (
                <div className="px-2 pb-2 space-y-0.5">
                  {mergedBranches.map((branch) => (
                    <div
                      key={branch.id}
                      className="flex items-center gap-2.5 px-2.5 py-1.5 rounded-sm text-xs text-muted-foreground/40"
                    >
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-1.5">
                ...
                <Button
                  ...
                  disabled={
                    creatingBranch || !newBranchName.trim() || !currentBranch
                  }
                >
                  {creatingBranch ? (
                    <Loader2 className="h-3.5 w-3.5 animate-spin" />
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-3 pt-1">
            ...
            <p className="text-xs text-muted-foreground/80">
              The main branch data will be replaced with the branch data. The
              source branch will be marked as merged.
            </p>
