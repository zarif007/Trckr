...
import type {
  ...
  TrackerResponse,
  ToolCallEntry,
} from "@/app/tracker/hooks/useTrackerChat";
...
function renderStreamingStatus(message: string) {
  return (
    <div className="w-full min-w-0 rounded-sm border border-border/40 bg-muted/40 px-4 py-3 flex items-center gap-3">
...
export function TrackerMessageList({
  ...
  const object = streamedObject as
    | {
        manager?: {
          thinking?: string;
          prd?: { name?: string; description?: string };
          builderTodo?: Array<{
            action?: string;
            target?: string;
            task?: string;
      ...
      {visibleMessages.map(({ message, index }) => (
        ...
        >
          {message.role === "assistant" && (
            <div className="w-8 h-8 rounded-sm bg-foreground flex items-center justify-center shrink-0 mt-0.5">
                ...
                <div className="w-full min-w-0 space-y-2">
                  {message.managerData && (
                    <div className="w-full min-w-0">
                      <div className="flex flex-col gap-2 p-3 rounded-sm bg-muted/40 border border-border/30 min-w-0 w-full">
                        <button
                          type="button"
                          onClick={() => {
                            setMessageThinkingOpen(
                        ...
                        {message.isThinkingOpen && (
                          ...
                          >
                            <div className="pt-2 space-y-3 min-w-0 w-full">
                              ...
                              {message.managerData.builderTodo &&
                                message.managerData.builderTodo.length > 0 && (
                                  <div className="mt-2 space-y-1.5 min-w-0 w-full">
                                    <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                                      Execution plan
                                    </p>
                                    <div className="flex flex-col gap-1 w-full min-w-0">
                                      {message.managerData.builderTodo.map(
                                        (todo, i) => (
                                          <div
                                            key={i}
                                            className="flex items-start gap-2 text-xs font-medium text-foreground/90 bg-background/40 p-2 rounded-sm border border-border/20 min-w-0 w-full"
                                          >
                                            <div
                    ...
                    <div className="w-full min-w-0">
                      <div className="flex flex-col gap-2 p-3 rounded-sm bg-muted/40 border border-border/30 min-w-0 w-full">
                        <button
                          type="button"
                          onClick={() =>
                            setMessageToolsOpen(index, !message.isToolsOpen)
                          }
                          className="flex items-center justify-between w-full text-xs font-semibold text-muted-foreground hover:text-foreground transition-colors"
                        >
                          <div className="flex items-center gap-2">
                        ...
                        {message.isToolsOpen && (
                          ...
                          >
                            <ToolCallProgress
                              ...
                              className="border-0 bg-transparent p-0 pt-2 rounded-none"
                            />
                          </motion.div>
                        )}
                      </div>
          ...
          {message.role === "user" && (
            <div className="w-8 h-8 rounded-sm bg-muted/60 border border-border/40 flex items-center justify-center shrink-0 mt-0.5">
        ...
        >
          <div className="flex gap-3 justify-start">
            ...
            <div className="flex-1 min-w-0 w-full space-y-3">
              <div className="flex items-center gap-2.5 rounded-sm px-4 py-2.5 bg-muted/60 border border-border/40 text-foreground text-sm font-medium">
              ...
              {isAnalystMode &&
                ...
                (object as { content?: string } | undefined)?.content && (
                  <motion.div
                    ...
                    className="rounded-sm px-4 py-2.5 bg-muted/60 border border-border/40 text-foreground"
                  >
                    <div className="prose prose-sm dark:prose-invert max-w-none prose-p:my-1.5 prose-headings:mt-3 prose-headings:mb-1.5 prose-ul:my-1.5 prose-ol:my-1.5 prose-li:my-0.5 prose-table:my-2 prose-pre:my-2">
                      <Markdown>
                        {(object as { content?: string }).content}
                      </Markdown>
                    </div>
                  </motion.div>
                )}
                  ...
                  <div className="w-full min-w-0 space-y-2">
                    {isLoading && object?.manager && (
                      <motion.div
                        initial={{ opacity: 0, scale: 0.98 }}
                        animate={{ opacity: 1, scale: 1 }}
                        className="p-4 rounded-sm bg-muted/30 border border-border/30 space-y-3 min-w-0 w-full"
                      >
                        <div className="flex items-center gap-2">
                        ...
                        {object.manager.builderTodo &&
                          object.manager.builderTodo.length > 0 && (
                            <motion.div
                            ...
                            >
                              ...
                              <div className="flex flex-col gap-1.5 w-full min-w-0">
                                {object.manager.builderTodo.map((todo, i) => (
                                  <div
                                    key={i}
                                    className="flex items-center gap-3 text-xs font-medium text-foreground bg-background/40 p-2 rounded-sm border border-border/20 min-w-0 w-full"
                                  >
                                    {todo?.action === "create" && (
