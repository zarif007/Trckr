...
export const MapFieldsNodeUI = memo(function MapFieldsNodeUI({
  id,
  data,
}: {
  id: string;
  ...
  const style = NODE_STYLES.map_fields;
  const mappingCount = (nodeData?.config?.mappings as Array<unknown>)?.length ?? 0;
    ...
    <div className={cn(NODE_BASE_CLASSES, "w-[200px]")}>
      ...
      <div className={NODE_BODY_CLASSES}>
        <div className="text-[11px] text-muted-foreground">
          {mappingCount} mapping{mappingCount !== 1 ? "s" : ""} configured
        </div>
      </div>
      <Handle
        type="target"
