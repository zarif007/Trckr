...
export const LANDING_DEMO_EXPR_FIELDS: AvailableField[] = [
  { fieldId: 'logic_lines_grid.logic_item_label', label: 'Line item', dataType: 'string' },
  ...
  { fieldId: 'project_list.project_name', label: 'Project', dataType: 'string' },
  ...
  { fieldId: 'project_list.project_due_date', label: 'Due date', dataType: 'date' },
  ...
  { fieldId: 'project_list.project_priority', label: 'Priority', dataType: 'options' },
  { fieldId: 'project_list.project_status', label: 'Status', dataType: 'options' },
]
...
const totalBudgetFormatted = LANDING_DEMO_REPORT_RECIPE_TOTAL_BUDGET.toLocaleString('en-US')
const highBudgetFormatted = LANDING_DEMO_REPORT_HIGH_PRIORITY_BUDGET.toLocaleString('en-US')
const mediumBudgetFormatted = LANDING_DEMO_REPORT_MEDIUM_PRIORITY_BUDGET.toLocaleString('en-US')
...
const analysisBarChartData = LANDING_DEMO_REPORT_ROWS.map((r) => ({
  ...
  budget:
    typeof r.sum_budget === 'number' && Number.isFinite(r.sum_budget) ? r.sum_budget : 0,
}))

const inProgressBudgetRow = LANDING_DEMO_REPORT_ROWS.find((r) => r.project_status === 'In Progress')
const blockedBudgetRow = LANDING_DEMO_REPORT_ROWS.find((r) => r.project_status === 'Blocked')
const inProgressBudgetFormatted =
  typeof inProgressBudgetRow?.sum_budget === 'number' && Number.isFinite(inProgressBudgetRow.sum_budget)
    ? inProgressBudgetRow.sum_budget.toLocaleString('en-US')
    : '—'
const blockedBudgetFormatted =
  typeof blockedBudgetRow?.sum_budget === 'number' && Number.isFinite(blockedBudgetRow.sum_budget)
...
3. **Re-run** after any pipeline edit so reports and analysis stay wired to the same query plan.`,
      ...
      chartData: [
        ...
        { priority: 'Medium', budget: LANDING_DEMO_REPORT_MEDIUM_PRIORITY_BUDGET },
      ],
    ...
    },
    {
      ...
      sources: 'Demo narrative · aligned with landing pipeline seed and report snapshot',
    },
  ],
}
