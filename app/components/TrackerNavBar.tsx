          ...
          <div className="relative flex items-center gap-2">
            ...
            {hasApplyStatusMenu && (
              <div className="relative flex items-center gap-1.5">
                <Popover open={applyStatusOpen} onOpenChange={setApplyStatusOpen}>
                  ...
                  <PopoverContent className="w-[min(100vw-2rem,16rem)] p-1" align="end" sideOffset={6}>
                    <div className="flex max-h-[min(60vh,320px)] flex-col gap-0.5 overflow-y-auto" role="menu">
                      {formActions.map((action) => {
                        const disabledOption = isAgentBuilding || formActionSaving
                        return (
                          <Button
                            key={action.id}
                            type="button"
                            ...
                            onClick={() => {
                              void Promise.resolve(onFormActionSelect?.(action)).finally(() =>
                                setApplyStatusOpen(false)
                              )
                            }}
                          >
                            {action.label}
                ...
                <DialogContent className="flex max-h-[min(92vh,720px)] w-[min(96vw,920px)] flex-col gap-0 overflow-hidden p-0">
                  <DialogHeader className="shrink-0 border-b border-border/60 px-6 pb-4 pt-6">
                    ...
                    <DialogDescription className="text-[13px] leading-relaxed">
                      The first card is the starting status. Every configured card appears in the Action menu in the
                      nav: choosing one saves to the database and sets that status (you can pick any action again at
                      any time). Optionally mark one row as Last to hide the Action menu after that status is reached.
                      Use “Fields editable in this status” to control whether the form stays editable when that status
                      is active.
                    </DialogDescription>
                  </DialogHeader>
                  <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
                    <div className="space-y-5">
                      {actionsDraft.map((action, index) => (
                        <div
                          key={action.id}
                          className="rounded-lg border border-border/70 bg-muted/10 p-4 shadow-sm"
                        >
                          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                              {index === 0 ? 'Starting status' : `Transition ${index}`}
                            </p>
                            {index > 0 ? (
                              <Button
                                type="button"
                              ...
                              <span className="text-[11px] text-muted-foreground">Required · always editable</span>
                            )}
                          </div>
                          <div className="grid gap-4 sm:grid-cols-2">
                            ...
                            <div className="space-y-1.5">
                              <span className="text-xs font-medium text-foreground">Status tag</span>
                              <Input
                              ...
                              <Checkbox
                                ...
                                onCheckedChange={(checked) => {
                                  ...
                                  if (checked === true) {
                                    ...
                                    updateDraftAction(action.id, { isLast: false })
                                  }
                                }}
                                disabled={index === 0}
                              />
                              <span>Last — hide Action menu after this status</span>
                            </label>
                          </div>
                        </div>
                      ))}
                    </div>
                    <div className="mt-6 rounded-lg border border-dashed border-border/80 bg-muted/5 p-4">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <p className="text-sm text-muted-foreground">
                          Add another transition users can pick from the Action menu.
                        </p>
                        <Button
                          type="button"
                          variant="secondary"
            ...
            {!autosaveEnabled &&
              ...
              ((onSaveTracker && !onFormActionSelect) ||
                (Boolean(onSaveData) && !hasApplyStatusMenu && !formActionsLocked)) && (
                <Button
                  variant="outline"
                  size="sm"
                  className="h-8 gap-1.5 text-xs"
