...
function mapMessages(conversation: {
  messages: Array<{
    id: string;
    role: Role;
    content: string;
...
export async function GET(
  ...
  if (conversationIdParam) {
    ...
    if (!conversation || conversation.trackerSchemaId !== trackerId) {
  ...
  type ConvWithMessages = Parameters<typeof mapMessages>[0] & {
    id: string;
    title: string | null;
    mode: ConversationMode;
    createdAt: Date;
  ...
  const conversations = (
    tracker as unknown as { conversations?: ConvWithMessages[] }
  )?.conversations;
