...
export async function runGenerateExprIntent(params: {
  ...
  const availableFields = deriveAvailableFields(
    params.currentTracker ?? null,
    gridId,
  );
  return generateExpr({
...
export async function executeGenerateValidationExpression(
  raw: unknown,
): Promise<GenerateExprResult> {
...
export async function executeGenerateCalculationExpression(
  raw: unknown,
): Promise<GenerateExprResult> {
  const input = exprIntentToolInputSchema.parse(raw);
  return runGenerateExprIntent({
