...
export async function POST(request: Request) {
  ...
  try {
    ...
    const effectiveDirty =
      dirtyFlag ?? inferTrackerDirtyFromPayload(currentTracker);
    ...
    if (parsed.masterDataScope) {
      masterDataScope =
        normalizeMasterDataScope(parsed.masterDataScope) ?? undefined;
    }
    ...
    const promptInputs: PromptInputs = {
      ...
      currentStateBlock,
      hasFullTrackerStateForPatch:
        hasFullTrackerStateForPatch(trackerForPrompt),
      conversationContext,
      hasMessages,
      masterDataScope,
    };
      ...
      async start(controller) {
        try {
          logAiStage(
            logContext,
            "orchestrate-start",
            "Starting multi-agent build-tracker.",
          );
          const baseTrackerForPostprocess =
            hasFullTrackerStateForPatch(trackerForPrompt) &&
            trackerForPrompt &&
            typeof trackerForPrompt === "object"
    ...
    return jsonError(
      message || "Failed to generate tracker. Please try again.",
      500,
    );
  }
}
