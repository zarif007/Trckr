...
export async function runManagerAgent(
  ...
  try {
    ...
    for await (const partial of streamResult.partialObjectStream) {
      write({ t: 'manager_partial', partial: partial as Partial<ManagerSchema> })
    }

    if (finishUsage) opts.onLlmUsage?.(finishUsage)
    if (opts.logContext) {
      logAiStage(opts.logContext, 'manager-stream-complete', 'Manager stream finished.')
    }
  ...
  try {
    ...
    if (opts.logContext) {
      logAiStage(opts.logContext, 'manager-fallback-complete', 'Manager fallback finished.')
    }
  ...
  opts.onLlmUsage?.(usage)
  if (opts.logContext) {
    logAiStage(opts.logContext, 'manager-retry-complete', 'Manager minimal-prompt retry finished.')
  }
  write({ t: 'manager_complete', manager: object })
  return object
}
