...
export async function POST(request: Request) {
  ...
  if (
    !body.tracker ||
    typeof body.tracker !== "object" ||
    Array.isArray(body.tracker)
  ) {
  ...
  const masterDataScope =
    ...
    normalizeMasterDataScope(
      (body.tracker as Record<string, unknown>).masterDataScope,
    ) ??
  ...
  const result = await applyMasterDataBindings({
    ...
    masterDataTrackers: body.masterDataTrackers as
      | MasterDataTrackerSpec[]
      | undefined,
    projectId: project.id,
    moduleId,
    userId: authResult.user.id,
