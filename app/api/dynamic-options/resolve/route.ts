...
import {
  getErrorMessage,
  parseResolveDynamicOptionsRequest,
} from "./lib/validation";
...
const rateLimitByFunctionId = new Map<
  string,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body == null)
      return badRequest("Invalid request body for dynamic options resolve");
    ...
    if (isRateLimited(parsed.data.functionId)) {
      return jsonError(
        "Rate limit exceeded for dynamic option resolution",
        429,
      );
    }
    ...
    const result = await resolveDynamicOptions({
      ...
      context: parsed.data.context as Parameters<
        typeof resolveDynamicOptions
      >[0]["context"],
