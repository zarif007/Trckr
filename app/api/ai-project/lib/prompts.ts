...
export function buildTrackerBuilderPrompt(input: {
  project: { name: string; description?: string; industry?: string; goals?: string[] }
  ...
  const goals = input.project.goals?.length ? input.project.goals.join(', ') : 'None'
  const modules = input.modules?.length
    ? input.modules.map((m) => `- ${m.name}${m.description ? `: ${m.description}` : ''}`).join('\n')
    : '- (none)'
