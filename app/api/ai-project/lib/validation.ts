...
export function parseBuildBody(body: unknown): BuildParseResult {
 ...
 return {
 ok: false,
 error: 'Invalid request body. Expected JSON with "projectId" and "trackerSpec".',
 status: 400,
 }
 }
 ...
 if (trackerSpec == null || typeof trackerSpec !== 'object' || Array.isArray(trackerSpec)) {
 return { ok: false, error: 'trackerSpec must be an object.', status: 400 }
 }

 if (projectContext != null && (typeof projectContext !== 'object' || Array.isArray(projectContext))) {
 return { ok: false, error: 'projectContext must be an object.', status: 400 }
 }
 ...
 moduleId: typeof moduleId === 'string' && moduleId.trim() ? moduleId.trim() : null,
