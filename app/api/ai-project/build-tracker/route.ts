...
function extractProgressFromPartial(partial: unknown): string | null {
  ...
  const fields = tracker.fields as
    | Array<{ ui?: { label?: string }; id?: string }>
    | undefined;
  if (fields?.length) {
  ...
  if (tracker.tabs && Array.isArray(tracker.tabs) && tracker.tabs.length)
    return "Adding tabs...";
  if (
    tracker.sections &&
    Array.isArray(tracker.sections) &&
    tracker.sections.length
  )
    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          if (
            partialStream &&
            typeof partialStream[Symbol.asyncIterator] === "function"
          ) {
            for await (const partial of partialStream) {
              ...
              if (msg && msg !== lastProgress) {
                ...
                controller.enqueue(
                  encoder.encode(
                    JSON.stringify({ type: "progress", message: msg }) + "\n",
                  ),
                );
              }
            }
          ...
          if (!trackerSchema) {
            ...
            controller.enqueue(
              encoder.encode(
                JSON.stringify({
                  type: "error",
                  message: "AI did not return a tracker schema.",
                }) + "\n",
              ),
            );
            controller.close();
          ...
          const resolvedName =
            trackerSpec.name?.trim() ||
            (trackerSchema as { name?: string }).name ||
            "Untitled tracker";
          let createdTracker: {
            id: string;
            name: string | null;
          ...
          try {
            ...
            if (masterDataScope === "tracker") {
              const grids = Array.isArray(
                (schemaWithScope as Record<string, unknown>).grids,
              )
                ? ((schemaWithScope as Record<string, unknown>).grids as Array<{
                    id?: string;
                  }>)
                : [];
              const tabs = Array.isArray(
                (schemaWithScope as Record<string, unknown>).tabs,
              )
                ? ((schemaWithScope as Record<string, unknown>).tabs as Array<{
                    id?: string;
                  }>)
                : [];
