...
function isSuccessSystemLine(content: string): boolean {
  ...
  return SUCCESS_PATTERNS.some(
    (p) => lower.includes(p.toLowerCase()) || content.includes(p),
  );
}
...
function isStatusValue(value: string): boolean {
  ...
  return STATUS_VALUES.some(
    (s) => v === s || v.startsWith(s + " ") || v.endsWith(" " + s),
  );
}
...
function getPanelStatus(lines: ClaiLine[]): PanelStatus {
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    if (c.includes("running") || c.includes("working") || c.includes("loading"))
      return "working";
...
function StatusChip({ status }: { status: PanelStatus }) {
  ...
  return (
    <span
      className="inline-flex items-center gap-2 rounded-sm border px-2.5 py-1 text-[11px] tracking-wide"
    ...
    >
      <div className="min-w-0">
        <div
          ...
          style={{ color: cli.text }}
        >
          CLAI Control Panel
        </div>
        <div
          className="mt-0.5 text-[11px] leading-4"
          style={{ color: cli.textMuted }}
        >
...
function CardShell({
  ...
  return (
    <div
      ...
      style={{ borderColor: cli.border, backgroundColor: cli.panelBg }}
    >
...
function LineBlock({ line }: { line: ClaiLine }) {
        ...
        <CardShell accent={cli.success} className="bg-transparent">
          <div className="flex items-start gap-2 font-mono text-[13px] leading-relaxed">
            <span
              ...
              className="select-none shrink-0 pt-[1px]"
            >
              $
            </span>
  ...
  return (
    <div className="animate-in fade-in duration-200">
      <div
        className="rounded-sm border px-4 py-4 font-mono text-[13px] leading-[1.65]"
        style={{
          color: cli.text,
          backgroundColor: cli.panelBg,
          borderColor: cli.border,
    ...
    >
      ...
      >
        <form onSubmit={handleSubmit} className="flex flex-col gap-2">
          <div
            className="flex items-center gap-2 rounded-sm border px-3 py-2 transition-colors"
            style={{
              borderColor: cli.border,
              backgroundColor: "rgba(255,255,255,0.02)",
            }}
          >
            <Input
              ...
              autoFocus
              className={cn(
                "min-w-0 flex-1 border-0 bg-transparent px-0 py-1.5 text-[13px] leading-6 ",
