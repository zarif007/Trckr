...
export function ProjectContent({
        ...
        queryClient.setQueryData<Project>(
          ...
          (prev) => {
            ...
            if (kind === "module") {
              return {
                ...prev,
                modules: updateModuleInTree(prev.modules, id, (m) => ({
                  ...m,
                  name,
                })),
              };
    ...
    async (item: ContextMenuItem) => {
      if (item.kind === "project") {
        ...
        queryClient.removeQueries({
          queryKey: dashboardQueryKeys.project(item.id),
        });
        queryClient.invalidateQueries({
      ...
      if (item.kind === "project" && project?.id === item.id) {
        ...
        return () => {
          queryClient.setQueryData(
            dashboardQueryKeys.project(item.id),
            deleted,
          );
        ...
        queryClient.setQueryData<Project>(
          ...
          (prev) =>
            prev
              ? {
                  ...prev,
                  modules: removeModuleFromTree(prev.modules, item.id),
                }
              : prev,
        );
        setProjects((prev) =>
  ...
  const tableRows = useMemo(() => {
    ...
    const moduleRows = modules.map((mod) => {
      const trackerCount = mod.trackerSchemas.filter(
        (t) => t.type === "GENERAL",
      ).length;
      return {
