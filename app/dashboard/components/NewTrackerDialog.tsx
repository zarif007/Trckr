...
const INSTANCE_OPTIONS: Array<{
...
}> = [
  {
    ...
    icon: Table2,
    description:
      "One shared dataset. Optional version control (branches, merge).",
  },
  {
    value: "MULTI",
    label: "Multi",
...
function SettingsToggleRow({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        "flex items-center justify-between rounded-sm border px-3 py-2 text-left transition-all duration-150 w-full",
        checked
    ...
    >
      ...
      <div
        aria-hidden="true"
        className={cn(
          "flex-shrink-0 w-9 h-5 rounded-sm transition-colors duration-200 relative",
          checked ? "bg-primary" : "bg-muted-foreground/30",
        )}
      >
        <span
          className={cn(
            "pointer-events-none absolute top-1/2 h-4 w-4 -translate-y-1/2 rounded-full bg-primary-foreground transition-[left] duration-200 ease-out",
...
export function NewTrackerDialog({
  ...
  const [masterDataScope, setMasterDataScope] =
    useState<MasterDataScope>("tracker");
  ...
  const scenarioA = Boolean(
    projectId && !hasInheritedDefault && !scopeDefaultLoading,
  );
  ...
  const showUpdateDefaultCheckbox = Boolean(
    projectId && scenarioB && scopeChangedFromBaseline,
  );
  ...
  const handleSubmit = useCallback(
    async (e?: React.FormEvent) => {
      ...
      try {
        ...
        const msg =
          err instanceof Error ? err.message : "Error creating tracker";
        setError(msg);
        onError?.(msg);
      } finally {
        setCreating(false);
  ...
  const instanceHelp =
    INSTANCE_OPTIONS.find((o) => o.value === instance)?.description ?? "";
  ...
  const scopeCallout = (() => {
    if (scopeDefaultLoading && projectId) {
      return (
        <div className="flex items-center gap-2 rounded-sm border border-border/60 bg-muted/30 px-2.5 py-1.5 text-[11px] text-muted-foreground">
    ...
    if (!projectId) {
      return (
        <div className="rounded-sm border border-border/60 bg-muted/30 px-2.5 py-1.5 text-[11px] text-muted-foreground leading-snug">
        ...
        <DialogTrigger asChild>
          {trigger ?? (
            <Button
              size="sm"
              variant="ghost"
              className="h-7 gap-1.5 rounded-sm text-xs font-medium"
            >
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <p className="text-[11px] text-muted-foreground">
                  Duplicate names get a suffix like{" "}
                  <span className="font-mono">(1)</span>.
                </p>
              </div>

              <div className="rounded-sm border border-border/80 bg-muted/20 p-4 space-y-5">
                <div className="space-y-2">
                  ...
                  <Tabs
                    value={instance}
                    onValueChange={(v) =>
                      handleInstanceChange(v as InstanceType)
                    }
                    className="gap-2"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={masterDataScope}
                    onValueChange={(v) =>
                      setMasterDataScope(v as MasterDataScope)
                    }
                  ...
                  >
                    ...
                    >
                      <SelectValue
                        placeholder={
                          scopeDefaultLoading ? "Loading…" : "Choose scope"
                        }
                      />
                    </SelectTrigger>
                    <SelectContent
                    ...
                    >
                      {SCOPE_OPTIONS.map((opt) => {
                        const moduleChoiceDisabled =
                          opt.value === "module" && !moduleId;
                        const itemDisabled =
                          moduleChoiceDisabled ||
                          (!projectId && opt.value !== "tracker");
                        return (
                          <SelectItem
                            key={opt.value}
                            value={opt.value}
                  ...
                  {!moduleId && projectId ? (
                    <p className="text-[11px] text-muted-foreground">
                      Module scope appears when you create a tracker from inside
                      a module.
                    </p>
                  ) : null}
                  {showSetDefaultCheckbox ? (
                  ...
                  {showUpdateDefaultCheckbox ? (
                    <label className="grid cursor-pointer grid-cols-[auto_1fr] items-start gap-x-3 gap-y-0">
                      ...
                      <span className="text-[11px] text-muted-foreground leading-snug pt-px min-w-0">
                        Update default {ownerKindLabel} setting to &ldquo;
                        {masterDataScope}&rdquo;
                      </span>
                    </label>
                  ) : null}
                </div>
                ...
                >
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    >
                      <div className="flex items-start gap-1.5 rounded-sm border border-primary/20 bg-primary/5 px-2.5 py-1.5">
                        <Info className="mt-0.5 h-3 w-3 shrink-0 text-primary" />
                        <p className="text-[10px] leading-snug text-primary/80">
                          A <strong>main</strong> branch is created on first
                          save. Use the tracker toolbar to branch and merge.
                        </p>
                      </div>
                    </div>
                    <SettingsToggleRow
                      ...
                      disabled={
                        creating || instance === "MULTI" || versionControl
                      }
                      onToggle={() => {
              ...
              {error ? (
                <p
                  className="text-xs text-destructive bg-destructive/10 px-3 py-2 rounded-sm"
                  role="alert"
                >
                  {error}
                </p>
          ...
          <DialogFooter className="shrink-0 flex-col items-stretch gap-3 border-t bg-muted/10 px-6 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
            ...
            <div className="flex justify-end gap-2 shrink-0">
              ...
              <Button
                ...
                className="min-w-[100px] gap-1.5 rounded-sm"
              >
                {creating ? (
                  <>
