...
export function DashboardUserMenu({
  ...
  if (status === "loading") {
    return (
      <div
        className={cn("shrink-0 rounded-sm bg-muted/50 animate-pulse", t.box)}
        aria-hidden
      />
    );
  }
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          type="button"
          className={cn(
            "flex shrink-0 cursor-pointer items-center justify-center overflow-hidden rounded-sm border border-border/50 bg-muted/30 outline-none transition-colors",
            t.box,
        ...
        >
          {user.image ? (
            ...
            <span className={cn("font-semibold text-foreground/80", t.initial)}>
              {initial}
            </span>
          )}
        </button>
      ...
      <PopoverContent
        ...
        sideOffset={6}
        className={cn(
          "w-60 overflow-hidden border bg-popover p-0 ",
        ...
        <div className="flex items-center gap-2.5 border-b border-border/60 px-3 py-2.5">
          {user.image ? (
            <Image
              ...
              width={36}
              height={36}
              className="h-9 w-9 shrink-0 rounded-sm object-cover ring-1 ring-border/50"
            />
          ) : (
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-sm border border-border/60 bg-muted/40 text-sm font-semibold text-foreground">
              {initial}
            </span>
          )}
