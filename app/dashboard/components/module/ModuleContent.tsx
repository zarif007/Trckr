...
const ALL_FILE_TYPES: SystemFileType[] = [
  "TEAMS",
  "SETTINGS",
  "RULES",
  "CONNECTIONS",
...
export function ModuleContent({
  ...
  const moduleAnalyses = useMemo(
    () =>
      (projectForReports?.analyses ?? []).filter(
    ...
    async (item: ContextMenuItem) => {
      if (item.kind === "module") {
        ...
        queryClient.removeQueries({
          queryKey: dashboardQueryKeys.module(item.id),
        });
        invalidateModuleAndProjects();
        await fetchProjects();
        if (item.id === moduleId) {
        ...
        const res = await fetch(`/api/trackers/${item.id}`, {
          method: "DELETE",
        });
        ...
        if (kind === "module" && mod) {
          if (mod.id === id) {
            queryClient.setQueryData<Module>(
              dashboardQueryKeys.module(moduleId),
          ...
          setProjects((prev) =>
            prev.map((p) =>
              p.id !== projectId
                ...
                : {
                    ...p,
                    modules: updateModuleInTree(p.modules, id, (m) => ({
                      ...m,
                      name,
                    })),
                  },
    ...
    (item: ContextMenuItem): (() => void) | void => {
      if (item.kind === "module" && mod) {
        const deleted =
          mod.id === item.id
            ? mod
            : (findModuleInTree(mod.children, item.id) ?? null);
        if (!deleted) return;
        if (mod.id === item.id) {
          setProjects((prev) =>
            prev.map((p) =>
        ...
        return () => {
          if (mod.id === item.id) {
            queryClient.setQueryData(
              dashboardQueryKeys.module(item.id),
              deleted,
            );
            setProjects((prev) =>
              prev.map((p) =>
            ...
            queryClient.setQueryData<Module>(
              ...
              (prev) =>
                prev
                  ? { ...prev, children: [...prev.children, deleted] }
                  : prev,
            );
            setProjects((prev) =>
              prev.map((p) =>
      ...
      if (item.kind === "analysis" && projectForReports) {
        const analysis = (projectForReports.analyses ?? []).find(
          (a) => a.id === item.id,
        );
        if (!analysis) return;
        queryClient.setQueryData<Project>(
  ...
  const availableFileTypes = ALL_FILE_TYPES.filter(
  ...
  const tableRows = useMemo(() => {
    ...
    const moduleRows = childModules.map((child) => {
      const trackerCount = child.trackerSchemas.filter(
        (t) => t.type === "GENERAL",
      ).length;
      return {
    ...
    const trackerRows = trackerSchemas.map((tracker) => {
      ...
      const listHref = tracker.listForSchemaId
        ...
        : tracker.instance === "MULTI"
          ? `/tracker-list/${tracker.id}`
          : listCompanionByParent.get(tracker.id)
            ? `/tracker-list/${listCompanionByParent.get(tracker.id)}`
            : null;
      return {
        kind: "tracker" as const,
        id: tracker.id,
        label: getTrackerDisplayName(
          tracker.name,
  ...
  const displayError =
    errorMessage ?? (isError && error ? (error as Error).message : null);
          ...
          <div className="h-full min-h-0">
            {isEmpty ? (
              <ProjectEmptyStatePanel
                ...
                description="Add trackers, nested modules, analyses, or dashboards from the menu above."
              >
                <CreateDropdown
                  projectId={projectId}
                  moduleId={moduleId}
