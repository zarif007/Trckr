...
function moduleContainsActive(
  ...
  if (
    ...
    projectAnalyses.some((a) => a.id === currentAnalysisId && a.moduleId === mod.id)
  )
    return true
        ...
        >
          ...
          {(trackers.length > 0 ||
            ...
            <span className="text-xs text-muted-foreground/60 tabular-nums flex-shrink-0 w-9 text-right ml-auto">
              {trackers.length + children.length + moduleReports.length + moduleAnalyses.length}
            </span>
          )}
        </Link>
      </div>
...
function SidebarProject({
  ...
  const projectLevelAnalyses = projectAnalyses.filter((a) => a.moduleId == null)
