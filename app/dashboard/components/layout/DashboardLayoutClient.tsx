...
function SidebarModule({
  ...
  const hasExpandableContent =
    hasTrackers || hasChildModules || hasAnalyses;
        ...
        >
          ...
          {(trackers.length > 0 ||
            ...
            <span className="text-xs text-muted-foreground/60 tabular-nums flex-shrink-0 w-9 text-right ml-auto">
              {trackers.length + children.length + moduleAnalyses.length}
            </span>
          )}
        </Link>
      </div>
...
function SidebarProject({
  ...
  const hasChildren =
    hasModules || hasProjectTrackers || hasProjectAnalyses;
  const activeAnalysisBelongsToProject =
    currentAnalysisId != null &&
    projectAnalyses.some((a) => a.id === currentAnalysisId);
  const containsActive =
    project.id === currentProjectId || activeAnalysisBelongsToProject;
