    ...
    <div className="mx-auto max-w-4xl space-y-8 px-4 py-8">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Tokens from the model provider for your account, scoped to projects and trackers when known.
          Under each tracker, report AI is split out, then shown per report with the data tracker name.
        </p>
      </div>
              ...
              <tbody>
                {data.byTracker.map((row) => {
                  ...
                  const showNestedBreakdown =
                    hasReports || (hasOther && row.otherOnTracker.totalTokens < row.totalTokens)
                    ...
                    <Fragment key={row.trackerSchemaId}>
                      ...
                      {hasTokens(row.otherOnTracker) ? (
                        <tr className="border-b border-border/20 bg-muted/20">
                          <td className="py-1.5 pr-4 pl-6 text-muted-foreground">Other AI (this tracker)</td>
                      ...
                      {row.reportDetails.length > 0 ? (
                        <>
                          <tr className="border-b border-border/20 bg-muted/10">
                            <td className="py-1.5 pr-4 pl-6 font-medium text-muted-foreground">Reports</td>
                          ...
                          {row.reportDetails.map((detail, i) => (
                            ...
                            >
                              <td className="py-1.5 pr-4 pl-10">
                                <div className="font-medium text-foreground">{detail.dataTrackerName}</div>
                              ...
                              <td className="py-1.5 pr-4 tabular-nums">{formatTokens(detail.totalTokens)}</td>
                              <td className="py-1.5 pr-4 tabular-nums">{formatTokens(detail.promptTokens)}</td>
                              <td className="py-1.5 tabular-nums">{formatTokens(detail.completionTokens)}</td>
                            </tr>
                          ))}
                        </>
                      ) : null}
