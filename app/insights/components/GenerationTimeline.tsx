    ...
    <div className="mb-8 space-y-3">
      ...
      <ol className="space-y-2">
        {steps.map((s, idx) => {
          ...
          const artifactLine =
            s.artifactKind !== undefined
              ? artifactLabels[s.artifactKind]
              : undefined;
          return (
            <li
              key={`${s.phase}-${idx}`}
              className={cn(
                "rounded-sm border border-border/40 bg-muted/20 text-sm overflow-hidden",
                    ...
                    {s.deltas.length > 0 && (
                      <ul className="space-y-0.5 text-xs text-muted-foreground list-none pl-0 font-mono leading-snug opacity-90">
                        {s.deltas.map((d, i) => (
                          <li
                            key={i}
                            className="border-l-2 border-primary/25 pl-2"
                          >
                            {d}
                          </li>
                        ))}
...
export function GenerationTimelineStarting({
  running,
  emptySteps,
}: {
  running: boolean;
