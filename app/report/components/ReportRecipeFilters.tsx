...
const META_PATH_SUGGESTIONS = ['__createdAt', '__updatedAt', '__label', '__gridId']
...
type ReportRecipeFiltersProps = {
  ...
  onRowTimeFilterChange: (v: QueryPlanV1['load']['rowTimeFilter'] | null) => void
...
export function ReportRecipeFilters({
  ...
  const setDateEnabled = (on: boolean) => {
    if (on) {
      onRowTimeFilterChange(rowTimeFilter ?? { field: 'createdAt', preset: 'last_30_days' })
    } else {
  ...
  return (
    <details
      className="rounded-md border border-border/50 bg-background shadow-sm overflow-hidden group"
      ...
      <div className="px-4 py-4 space-y-5 text-sm">
        {requirement ? (
          <div className="rounded-md border border-border/40 bg-muted/15 px-3 py-2 space-y-1">
            ...
            <p className="text-xs text-foreground/90 whitespace-pre-wrap">{requirement}</p>
          </div>
        ) : null}
        {formatterOnlyGroupBy ? (
          <p className="text-xs text-muted-foreground rounded-md border border-border/40 bg-muted/20 px-3 py-2">
            This report groups data only in the formatter step. Change grouping with{' '}
            <strong className="font-medium text-foreground/80">Regenerate</strong>, or adjust filters below.
          </p>
        ) : null}

        <div className="rounded-md border border-border/50 bg-muted/20 p-3 sm:p-4 space-y-3">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
            <div className="flex gap-3 min-w-0">
              ...
              <div className="min-w-0 space-y-0.5">
                <p className="text-sm font-medium text-foreground">Row date range</p>
                <p className="text-xs text-muted-foreground leading-snug">
                  Only include tracker rows whose timestamp falls in this window. Turn off to load all dates (subject to
                  row limits).
                </p>
              </div>
            </div>
            <button
              ...
              aria-label={dateEnabled ? 'Disable date filter' : 'Enable date filter'}
              disabled={disabled}
            ...
            >
              <span
                className={cn(
                  'pointer-events-none h-5 w-5 rounded-full bg-background shadow-sm transition-[margin]',
          ...
          {dateEnabled && rowTimeFilter ? (
            <div className="rounded-md border border-border/40 bg-background/90 p-3 sm:p-4 space-y-4">
              <p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Configure range</p>
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <span className="text-xs font-medium text-foreground">Timestamp field</span>
                  <p className="text-[11px] text-muted-foreground">Which row time to compare</p>
                  <Select
                    ...
                    disabled={disabled}
                  >
                    <SelectTrigger size="sm" className="w-full rounded-md">
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="space-y-1.5">
                  <span className="text-xs font-medium text-foreground">Quick range</span>
                  <p className="text-[11px] text-muted-foreground">Preset or refine below</p>
                  <Select
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  <span className="text-xs font-medium text-foreground">Custom start</span>
                  <p className="text-[11px] text-muted-foreground">Optional; combines with preset when set</p>
                  <Input
                    type="datetime-local"
                    value={rowTimeFilter.from ? isoToDatetimeLocal(rowTimeFilter.from) : ''}
                    onChange={(e) => {
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  <span className="text-xs font-medium text-foreground">Custom end</span>
                  <p className="text-[11px] text-muted-foreground">Optional upper bound</p>
                  <Input
                    type="datetime-local"
                    value={rowTimeFilter.to ? isoToDatetimeLocal(rowTimeFilter.to) : ''}
                    onChange={(e) => {
                      const v = e.target.value
                      onRowTimeFilterChange({
                        ...rowTimeFilter,
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between gap-2">
            <span className="text-xs font-medium text-muted-foreground">Row filters</span>
            <Button
              type="button"
              variant="outline"
              size="sm"
              ...
              disabled={disabled}
              onClick={() =>
                onFilterRowsChange([...filterRows, { path: '', op: 'eq', valueRaw: '' }])
              }
            >
            ...
            <ul className="space-y-2">
              {filterRows.map((row, idx) => (
                <li
                  key={idx}
                  className={cn(
                    'grid gap-2 items-end sm:grid-cols-[1fr_minmax(0,7rem)_1fr_auto] p-2 rounded-md border border-border/40 bg-muted/10',
                  )}
                >
                  ...
                  <div className="space-y-1 min-w-0">
                    <span className="text-[10px] uppercase tracking-wide text-muted-foreground">Op</span>
                    <Select
                      value={row.op}
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="icon"
                    className="shrink-0 text-muted-foreground hover:text-destructive rounded-md"
                    disabled={disabled}
                    onClick={() => onFilterRowsChange(filterRows.filter((_, i) => i !== idx))}
        ...
        <div className="flex flex-col gap-3 pt-2 border-t border-border/40 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button
            type="button"
            size="sm"
            className="shrink-0 gap-2 self-start sm:self-auto rounded-md"
