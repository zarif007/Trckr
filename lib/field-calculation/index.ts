...
export function extractExprFieldRefs(
  expr: ExprNode,
  out = new Set<string>(),
): Set<string> {
  ...
  switch (expr.op) {
    ...
    case "lt":
    case "lte":
      if (isExprNode((expr as { left?: ExprNode }).left))
        extractExprFieldRefs(expr.left, out);
      if (isExprNode((expr as { right?: ExprNode }).right))
        extractExprFieldRefs(expr.right, out);
 ...
 */
export function getAccumulateSourceGridIds(
  expr: ExprNode,
  out = new Set<string>(),
): Set<string> {
  if (!isExprNode(expr)) return out;
  if (expr.op === "accumulate" || expr.op === "sum" || expr.op === "count") {
    const { gridId } = parsePath(
      (expr as { sourceFieldId: string }).sourceFieldId,
    );
  ...
  if (
    ...
    if (binary.left && isExprNode(binary.left))
      getAccumulateSourceGridIds(binary.left, out);
    if (binary.right && isExprNode(binary.right))
      getAccumulateSourceGridIds(binary.right, out);
    if (Array.isArray(binary.args)) {
      if (binary.args[0] && isExprNode(binary.args[0]))
        getAccumulateSourceGridIds(binary.args[0], out);
      if (binary.args[1] && isExprNode(binary.args[1]))
        getAccumulateSourceGridIds(binary.args[1], out);
    }
    return out;
  }
  if (expr.op === "not" && isExprNode(expr.arg))
    getAccumulateSourceGridIds(expr.arg, out);
  if (expr.op === "if") {
  ...
  if (expr.op === "regex" && isExprNode(expr.value))
    getAccumulateSourceGridIds(expr.value, out);
  return out;
}
...
function getImpactedTargets(
  ...
  if (!changedFieldIds || changedFieldIds.length === 0) {
    for (const targetFieldId of rulesByTargetFieldId.keys())
      impacted.add(targetFieldId);
    return impacted;
  }
...
export function compileCalculationsForGrid(
  ...
  const { sourceFieldsByTarget, reverseDeps } = buildDependencyGraph(
    gridId,
...
function getCachedCompiledPlan(
  ...
  if (byGrid) {
    ...
    compiledPlanCache.set(
      calculations,
      new Map<string, CompiledCalculationPlan>([[gridId, compiled]]),
    );
  }
...
export function applyCompiledCalculationsForRow({
  ...
  const normalizedChangedFieldIds = normalizeChangedFieldIds(
    changedFieldIds,
    plan.gridId,
  );
  ...
  const getColumnValues =
    gridData && Object.keys(gridData).length > 0
      ? buildGetColumnValues(gridData)
      : undefined;

  const { order, cyclicTargets } = resolveEvaluationOrder(
    plan.sourceFieldsByTarget,
    impactedTargets,
  );
  const nextRow = { ...row };
  ...
  for (const targetFieldId of order) {
    const nextValue = evaluateTarget(
      plan,
      rowValues,
      targetFieldId,
      getColumnValues,
