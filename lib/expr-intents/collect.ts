...
export function isIntentValidationRule(
  rule: unknown,
): rule is ValidationRuleWithIntent {
  if (!rule || typeof rule !== "object") return false;
  const r = rule as Record<string, unknown>;
  return r.type === "expr" && typeof r._intent === "string";
}

export function isIntentCalculation(
  entry: unknown,
): entry is CalculationWithIntent {
...
export function collectExprIntents(tracker: TrackerLike): ExprIntent[] {
  ...
  const calculations =
    (tracker as TrackerLike & { calculations?: Record<string, unknown> })
      .calculations ?? {};
