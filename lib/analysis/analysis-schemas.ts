...
export const analysisSectionSchema = z.object({
  ...
  focus: z
    .string()
    .describe(
      "What this section should cover; used when writing the analysis.",
    ),
...
export type AnalysisOutlinePayload = z.infer<
  typeof analysisOutlinePayloadSchema
>;

export function parseAnalysisOutlineOnly(
  data: unknown,
): AnalysisOutlineOnly | null {
  const r = analysisOutlineOnlySchema.safeParse(data);
  return r.success ? r.data : null;
}

export function parseAnalysisDocument(
  data: unknown,
): AnalysisDocumentV1 | null {
