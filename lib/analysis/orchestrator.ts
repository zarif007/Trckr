...
export type LoadedAnalysis = NonNullable<
  Awaited<ReturnType<typeof getAnalysisForUser>>
>;
...
function toOutlinePayload(
  outline: AnalysisOutlineOnly,
): AnalysisOutlinePayload {
  return {
    version: 1,
...
export async function executeAnalysisFullGeneration(params: {
  ...
  await withRun(analysis.id, trigger, writeNdjsonLine, async (forward) => {
    ...
    const hydratedBlocks = hydrateChartDataForBlocks(
      docParsed.blocks,
      rawResult,
    );
    const document: AnalysisDocumentV1 = { version: 1, blocks: hydratedBlocks };
