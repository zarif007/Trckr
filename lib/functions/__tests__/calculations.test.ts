...
describe("field calculations", () => {
  ...
  it("skips cyclic targets and still computes non-cyclic targets", () => {
    ...
    expect(result.skippedCyclicTargets).toEqual(
  ...
  it("getGridIdsThatDependOnGridViaAccumulate returns target grids that accumulate over source", () => {
    ...
    expect(
      getGridIdsThatDependOnGridViaAccumulate(calcs, "amounts_grid"),
    ).toEqual(["main_grid"]);
    expect(
      getGridIdsThatDependOnGridViaAccumulate(calcs, "other_grid"),
    ).toEqual([]);
    expect(
      getGridIdsThatDependOnGridViaAccumulate(undefined, "amounts_grid"),
    ).toEqual([]);
  });
  ...
  it("evaluates accumulate (sum table column) when gridData is provided", () => {
    const gridData = {
      amounts_grid: [{ amount: 10 }, { amount: 20 }, { amount: 30 }],
      main_grid: [{}],
    };
  ...
  it("compiled plan produces same output as wrapper API", () => {
    ...
    expect(fromCompiled.skippedCyclicTargets).toEqual(
      wrapped.skippedCyclicTargets,
    );
  });
});
...
describe("calculation validation", () => {
  it("rejects cross-grid field references in calculation expressions", () => {
    const tracker = {
      ...
      grids: [
        { id: "sales_grid", sectionId: "main_section" },
        { id: "other_grid", sectionId: "main_section" },
      ],
      fields: [
        { id: "amount", dataType: "number" },
        { id: "rate", dataType: "number" },
        { id: "qty", dataType: "number" },
      ],
      layoutNodes: [
    ...
    expect(
      (result.errors ?? []).some((e) =>
        e.includes("must stay within target grid"),
      ),
    ).toBe(true);
  });
  ...
  it("rejects invalid accumulate params (increment)", () => {
    ...
    const ctx = buildValidationContext(tracker as unknown as TrackerLike);
    const result = validateCalculations(ctx);
    expect((result.errors ?? []).some((e) => e.includes("increment"))).toBe(
      true,
    );
  });
  ...
  it("rejects invalid keys, missing targets, malformed expr, and dependency cycles", () => {
    const tracker = {
      ...
      fields: [
        { id: "amount", dataType: "number" },
        { id: "subtotal", dataType: "number" },
        { id: "tax", dataType: "number" },
      ],
      layoutNodes: [
        { gridId: "sales_grid", fieldId: "amount", order: 0 },
      ...
      calculations: {
        ...
        "sales_grid.subtotal": {
          expr: { op: "field", fieldId: "sales_grid.tax" },
        },
        "sales_grid.tax": {
          expr: { op: "field", fieldId: "sales_grid.subtotal" },
        },
      },
    };
    ...
    expect(
      (result.errors ?? []).some((e) => e.includes('must be "gridId.fieldId"')),
    ).toBe(true);
    expect((result.errors ?? []).some((e) => e.includes("field path"))).toBe(
      true,
    );
    expect(
      (result.errors ?? []).some((e) =>
        e.includes(".expr must be a valid expression node"),
      ),
    ).toBe(true);
    expect(
      (result.errors ?? []).some((e) => e.includes("dependency cycle")),
    ).toBe(true);
  });
});
...
describe("tracker patch merge for calculations", () => {
  const base: TrackerDisplayProps = {
    ...
    sections: [
      { id: "main_section", name: "Main", tabId: "main_tab", placeId: 0 },
    ],
    grids: [
      {
        id: "sales_grid",
