 ...
 */
export function buildRowValuesForReportRow(row: Record<string, unknown>): Record<string, unknown> {
  const out: Record<string, unknown> = { ...row }
  const gridId = typeof row.__gridId === 'string' && row.__gridId.trim() ? row.__gridId.trim() : null
  if (gridId) {
...
export function evaluateReportExprOnRow(expr: ExprNode, row: Record<string, unknown>): unknown {
