...
import {
  buildReportCalcUserPrompt,
  getReportCalcSystemPrompt,
} from "@/lib/prompts/report-calc";
import {
  buildReportFormatterUserPrompt,
  getReportFormatterSystemPrompt,
} from "@/lib/prompts/report-formatter";
import {
  buildReportIntentUserPrompt,
  getReportIntentSystemPrompt,
} from "@/lib/prompts/report-intent";
import {
...
function buildMarkdownPreamble(
  _intent: ReportIntent | null,
  _userPrompt: string,
): string {
  return "";
}
...
export async function executeReportReplay(params: {
  ...
  await withRun(report.id, trigger, writeNdjsonLine, async (forward) => {
    ...
    const preambleMarkdown = buildMarkdownPreamble(
      storedIntent,
      def.userPrompt,
    );
    const md =
...
export async function executeReportFullGeneration(params: {
  ...
  await withRun(report.id, trigger, writeNdjsonLine, async (forward) => {
    ...
    await forward({
      t: "phase_start",
      phase: "intent",
      label: "Mapping data requirements",
    });
    await forward({
      t: "phase_delta",
      phase: "intent",
    ...
    let intent: ReportIntent = {
      ...intentResult.object,
      generationPlan:
        intentResult.object.generationPlan ?? defaultReportGenerationPlan(),
    };
    ...
    const preCalcSampleJson = JSON.stringify(
      preCalcSchema.sample.slice(0, 15),
      null,
      2,
    );
    ...
    const primaryGrid = primaryGridIdForReport(
      intent,
      rawResult,
      catalog.gridIds,
    );
    ...
    await forward({
      t: "phase_end",
      phase: "formatter",
      summary: "Formatter plan validated.",
    });

    await forward({
      t: "phase_start",
      phase: "apply",
      label: "Applying formatter",
    });
    ...
    const md =
      ...
      formatOutputMarkdown(
        formattedRows,
        formatterPlan.outputStyle ?? "markdown_table",
        {
