...
function intentWithMetrics(metrics: ReportIntent["metrics"]): ReportIntent {
  return {
    narrative: "test",
    gridIds: [],
    metrics,
...
describe("userPromptImpliesNumericTotal", () => {
  it("matches total volume and sum wording", () => {
    expect(
      userPromptImpliesNumericTotal("What is the total volume of sales?"),
    ).toBe(true);
    expect(userPromptImpliesNumericTotal("combined revenue by quarter")).toBe(
      true,
    );
  ...
  it("does not match generic listing", () => {
    expect(
      userPromptImpliesNumericTotal("List open tasks with due dates"),
    ).toBe(false);
  });
...
describe("shouldRetryIntentForNumericTotalMismatch", () => {
  it("is true when prompt implies numeric total but metrics are count-only", () => {
    ...
    expect(
      shouldRetryIntentForNumericTotalMismatch(
        "Show total volume across all instances",
        intent,
      ),
    ).toBe(true);
  });
  ...
  it("is false when metrics include sum", () => {
    ...
    expect(
      shouldRetryIntentForNumericTotalMismatch("total volume", intent),
    ).toBe(false);
  });
