...
export const reportIntentSchema = z.object({
  ...
  metrics: z
    .array(
      z.object({
        label: z.preprocess(
          (v) => (typeof v === "string" ? v : "metric"),
          z.string(),
        ),
        aggregation: z.enum(["sum", "count", "avg", "min", "max", "none"]),
        fieldPath: z
          .preprocess((v) => (typeof v === "string" ? v : ""), z.string())
          .describe("Field id from the tracker schema catalog."),
      }),
    )
    .default([]),
    ...
    .array(
      z.object({
        ...
        value: z
          ...
          .describe(
            "Filter value: primitive, array for `in`, or a shallow object if needed.",
          ),
      }),
    )
    .default([]),
...
export function normalizeReportIntent(data: unknown): ReportIntent | null {
  ...
  const base = reportIntentSchema
    .omit({ generationPlan: true })
    .safeParse(data);
  if (!base.success) return null;
  return { ...base.data, generationPlan: defaultReportGenerationPlan() };
}

export function formatGenerationPlanForPrompt(
  plan: ReportGenerationPlan | undefined,
): string {
  if (
    !plan ||
    (plan.objectives.length === 0 && !plan.formatterGuidance.trim())
  ) {
