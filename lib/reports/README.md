...
1. **Intent** (`reportIntentSchema`) — LLM extracts metrics, grids, filters, time range, output style.
2. **Query plan** (`queryPlanV1Schema`) — LLM maps intent + field catalog to a safe AST (no SQL/JS).
3. **Execute** — `executeQueryPlan` loads rows via Prisma (`buildTrackerDataWhere`), flattens grid arrays from `TrackerData.data`, filters, optional aggregate, sort, limit. Aggregate **metrics** use either a single `path` or a per-row `expression` (same AST as formatter `compute_column`) for sums like **quantity × unit_price** — not `sum(unit_price)` for “total value”.
...
5. **Formatter plan** (`formatterPlanV1Schema`) — LLM sees column schema + sample **after** calc enrichment; emits ordered ops (`drop_columns`, `filter`, `sort`, `rename`, `limit`, `group_by`, `compute_column`).
...
Fingerprint is a short SHA-256 prefix of a stable JSON blob derived from the tracker field catalog (`fingerprintFromCatalog`).
