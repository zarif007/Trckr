...
/** Structured intent from the first LLM pass (recipe, not executable). */
export const reportIntentSchema = z.object({
  ...
  metrics: z.array(
    z.object({
      ...
      fieldPath: z.string().describe('Field id from the tracker schema catalog.'),
    }),
  ),
  groupByFieldPaths: z.array(z.string()),
  filters: z.array(
    z.object({
      fieldPath: z.string(),
      ...
      value: z
        .unknown()
        .describe('Concrete filter value: string, number, boolean, or array for `in`.'),
    }),
  ),
  timeRange: z.object({
    ...
    preset: z
      ...
      .optional(),
    /** Which TrackerData timestamp to filter before flattening. */
    applyToRow: z.enum(['createdAt', 'updatedAt']).optional(),
    fromIso: z.string().optional(),
    toIso: z.string().optional(),
  }),
  outputStyle: z.enum(['table', 'summary', 'both']),
})
...
/** Operand for row calculations: a flattened column path or numeric literal. */
...
/** Safe row-level expression (no arbitrary code). */
...
export type FormatterComputeExpression = z.infer<typeof formatterComputeExpressionSchema>

/** Query aggregate or formatter group_by metric: single column path OR per-row expression (not both). */
...
export const queryPlanV1Schema = z.object({
  version: z.literal(1),
  load: z.object({
    maxTrackerDataRows: z.number().int().min(1).max(5000),
    branchName: z.string().nullable().optional(),
    rowTimeFilter: rowTimeFilterSchema.optional(),
  }),
  flatten: z.object({
    gridIds: z.array(z.string()).describe('Empty = all grids that have row arrays in data.'),
  }),
  filter: z
    .array(
      z.object({
...
export const formatterPlanV1Schema = z.object({
  version: z.literal(1),
  outputStyle: z.enum(['markdown_table', 'markdown_summary', 'both']).default('markdown_table'),
  ops: z.array(formatterOpSchema).default([]),
})
