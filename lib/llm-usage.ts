...
export async function resolveLlmUsageAttribution(
  ...
  { ok: true; value: LlmUsageAttribution } | { ok: false; error: string; status: number }
> {
  ...
  if (tsId) {
    ...
    if (!row) {
      return { ok: false, error: 'Tracker not found or access denied.', status: 403 }
    }
    if (pId && row.projectId !== pId) {
      return { ok: false, error: 'Project does not match tracker.', status: 400 }
    }
    return { ok: true, value: { projectId: row.projectId, trackerSchemaId: row.id } }
  }
  ...
  if (pId) {
    ...
    if (!project) {
      return { ok: false, error: 'Project not found or access denied.', status: 403 }
    }
    return { ok: true, value: { projectId: project.id, trackerSchemaId: null } }
  }
...
export function scheduleRecordLlmUsage(params: Parameters<typeof recordLlmUsage>[0]): void {
...
export async function getLlmUsageDashboard(userId: string): Promise<LlmUsageDashboard> {
