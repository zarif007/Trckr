...
export function compareValues(rowVal: unknown, op: QueryPlanV1['filter'][number]['op'], value: unknown): boolean {
  switch (op) {
    case 'eq':
      return rowVal === value || (typeof rowVal === 'number' && rowVal === toNumeric(value))
    case 'neq':
      return !(rowVal === value || (typeof rowVal === 'number' && rowVal === toNumeric(value)))
    case 'gt':
    case 'gte':
    ...
    case 'in':
      return Array.isArray(value) && value.some((v) => compareValues(rowVal, 'eq', v))
    default:
      return false
  }
}
  ...
  for (const r of rows) {
    for (const gridId of useGrids) {
      ...
      for (const cell of arr) {
        ...
        idx += 1
        for (const [key, val] of Object.entries(cell as Record<string, unknown>)) {
  ...
  for (const [, groupRows] of map) {
    ...
    for (const m of metrics) {
      ...
      if (m.op === 'sum') {
        ...
        out[m.name] = nums.length ? nums.reduce((a, b) => a + b, 0) / nums.length : null
      } else if (m.op === 'min') {
...
export function executeQueryPlan(
  ...
  if (plan.aggregate && plan.aggregate.metrics.length > 0) {
    result = aggregateRows(flat, plan.aggregate.groupBy, plan.aggregate.metrics)
  } else {
    result = flat
  }
...
export function resultSchemaFromRows(
  rows: Record<string, unknown>[],
  sampleSize: number,
): { columns: { key: string; sampleTypes: string }[]; sample: Record<string, unknown>[] } {
  if (rows.length === 0) {
