...
const LABEL_PRIORITY = ['name', 'full_name', 'fullname', 'title', 'label', 'value']
...
export function resolveMasterDataGridId(
 ...
 const gridIds = grids.map((g) => g.id).filter((id): id is string => typeof id === 'string' && id.length > 0)
 if (gridIds.length === 0) return null

 const metaGridId = isPlainObject(schema.masterDataMeta) && typeof schema.masterDataMeta.gridId === 'string'
 ? (schema.masterDataMeta.gridId as string)
 : null
 const preferred = typeof preferredId === 'string' && preferredId.trim().length > 0 ? preferredId.trim() : null
...
export function extractMasterDataFields(
 ...
 const fallbackFieldIds = fields.map((f) => f.id).filter((id): id is string => typeof id === 'string')
...
export function resolveLabelFieldId(options: {
 ...
 const match = fieldIds.find((id) => normalizeToken(id) === normalizeToken(candidate))
...
export function computeMasterDataSignature(fieldIds: string[], fieldLabels: string[]): string {
 const ids = [...fieldIds].map(normalizeToken).filter(Boolean).sort().join('|')
 const labels = [...fieldLabels].map(normalizeToken).filter(Boolean).sort().join('|')
...
export function buildMasterDataMeta(options: {
 ...
 const { fieldIds, fieldLabels, fieldIdToLabel, gridId } = extractMasterDataFields(schema)
...
export function readMasterDataMeta(schema: Record<string, unknown>): MasterDataMeta | null {
 ...
 if (!Array.isArray(raw.fieldIds) || !Array.isArray(raw.fieldLabels)) return null
