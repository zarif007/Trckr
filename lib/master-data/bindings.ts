...
function stripLocalOptionsGrids(
  tracker: Record<string, unknown>,
): Record<string, unknown> {
  const grids = Array.isArray(tracker.grids) ? (tracker.grids as Grid[]) : [];
  const layoutNodes = Array.isArray(tracker.layoutNodes)
    ? (tracker.layoutNodes as LayoutNode[])
    : [];
  const fields = Array.isArray(tracker.fields)
    ? (tracker.fields as Field[])
    : [];
  const sections = Array.isArray(tracker.sections)
    ? (tracker.sections as Array<{ id?: string; tabId?: string }>)
    : [];
  const tabs = Array.isArray(tracker.tabs)
    ? (tracker.tabs as Array<{ id?: string }>)
    : [];

  const optionGridIds = new Set(
    grids
...
function stripLocalMasterDataGrids(
  ...
  const localForeignIds = new Set(
    grids.filter((g) => g.id && foreignGridIds.has(g.id)).map((g) => g.id!),
  );
  if (localForeignIds.size === 0) return tracker;

  const nextGrids = grids.filter((g) => !localForeignIds.has(g.id ?? ""));
  const nextLayoutNodes = layoutNodes.filter(
    (n) => !localForeignIds.has(n.gridId ?? ""),
  );
...
function stripModuleProjectMasterDataTab(
  tracker: Record<string, unknown>,
): Record<string, unknown> {
  const grids = Array.isArray(tracker.grids) ? (tracker.grids as Grid[]) : [];
  const layoutNodes = Array.isArray(tracker.layoutNodes)
  ...
  const sectionIdsToRemove = new Set(
    sections
      .filter((s) => s.tabId === MASTER_DATA_TAB_ID)
      .map((s) => s.id ?? "")
      .filter(Boolean),
  );
  ...
  const gridIdsToRemove = new Set(
    grids
      .filter((g) => sectionIdsToRemove.has(g.sectionId ?? ""))
      .map((g) => g.id ?? "")
      .filter(Boolean),
  );
  ...
  const nextLayoutNodes = layoutNodes.filter(
    (n) => !gridIdsToRemove.has(n.gridId ?? ""),
  );
  const usedFieldIds = new Set(
    nextLayoutNodes
      .map((n) => n.fieldId)
  ...
  const nextTabs = tabs.filter((t) =>
    t.id === MASTER_DATA_TAB_ID ? false : usedTabIds.has(t.id ?? ""),
  );
...
function ensureBindingEntry(options: {
  ...
  const fieldMappings = hasValueMapping
    ? normalizedMappings
    : [valueMapping, ...normalizedMappings];
  const existingKey =
    typeof existing?.optionsSourceKey === "string"
      ? existing.optionsSourceKey
      : undefined;
...
function resolveUnboundSelectFields(options: {
  ...
  for (const field of fields) {
    ...
    const existingBinding = bindings[fieldPath] as
      | Record<string, unknown>
      | undefined;
...
export async function applyMasterDataBindings(options: {
  ...
  const scope =
    normalizeMasterDataScope(options.scope) ??
    resolveMasterDataScopeFromTracker(tracker as { masterDataScope?: unknown });
  const nextTracker: Record<string, unknown> = {
    ...(tracker ?? {}),
  ...
  const layoutNodes = Array.isArray(nextTracker.layoutNodes)
    ? (nextTracker.layoutNodes as LayoutNode[])
    : [];
  const bindings = isPlainObject(nextTracker.bindings)
    ? { ...(nextTracker.bindings as Record<string, unknown>) }
    : {};
  ...
  for (const spec of specs) {
    ...
    if (spec.schema) {
      const gridId = resolveMasterDataGridId(
  ...
  const findCompatibleBySpec = (spec: MasterDataTrackerSpec) => {
    ...
    const usableSchema = desiredGridId
      ? desiredSchema
      : buildMasterDataSchema(spec.name);
    let desiredFields = extractMasterDataFields(
      usableSchema,
      desiredGridId,
    ).fieldIds;
    ...
    const compatible = candidates.find((t) =>
      desiredFields.every((id) => t.meta.fieldIds.includes(id)),
    );
  ...
  for (const entry of unresolvedFields) {
    ...
    const rawKey =
      typeof binding?.optionsSourceKey === "string"
        ? binding.optionsSourceKey.trim()
        : "";
    ...
    if (spec) {
      ...
      if (!targetCache.has(cacheKey)) {
        ...
        if (compatible) {
          const fieldsInfo = extractMasterDataFields(
            compatible.schema,
            compatible.meta?.gridId,
          );
          ...
          const gridId =
            fieldsInfo.gridId ??
            resolveMasterDataGridId(compatible.schema, compatible.meta?.gridId);
          if (!gridId) continue;
          ...
          actions.push({
            type: "reuse",
            name: compatible.name || spec.name,
            key: spec.key,
            trackerId: compatible.id,
          });
        } else {
    ...
    if (!targetCache.has(fallbackKey)) {
      ...
      if (!match) {
        ...
        const meta =
          readMasterDataMeta(schemaWithMeta) ??
          buildMasterDataMeta({ schema: schemaWithMeta });
        match = {
          id: created.id,
          name: created.name ?? "",
          normalized,
        ...
        actions.push({
          type: "create",
          name: created.name ?? entityName,
          trackerId: created.id,
        });
      } else {
        actions.push({
          type: "reuse",
          name: match.name || entityName,
          trackerId: match.id,
        });
      }
      ...
      const gridId =
        fieldsInfo.gridId ??
        resolveMasterDataGridId(match.schema, match.meta?.gridId);
      if (!gridId) continue;
