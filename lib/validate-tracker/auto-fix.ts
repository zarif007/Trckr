...
export function autoFixBindings<T extends TrackerLike>(tracker: T): T {
  ...
  for (const [fieldPath, entry] of Object.entries(fixed.bindings!)) {
    ...
    const existingInOptGrid = fixed.layoutNodes!.filter(
      (n) => n.gridId === optGridId && n.fieldId === labelFieldId,
    );
    const hasNewNode = fixed.layoutNodes!.some(
      (n) => n.gridId === optGridId && n.fieldId === optionFieldId,
    );
    if (existingInOptGrid.length > 0 && !hasNewNode) {
      const maxOrder = Math.max(
        0,
        ...fixed
          .layoutNodes!.filter((n) => n.gridId === optGridId)
          .map((n) => n.order ?? 0),
      );
      fixed.layoutNodes = fixed.layoutNodes!.filter(
        (n) => !(n.gridId === optGridId && n.fieldId === labelFieldId),
      );
      fixed.layoutNodes!.push({
        gridId: optGridId,
  ...
  let gridPlaceId = Math.max(
    0,
    ...fixed
      .grids!.filter((g) => g.sectionId === container.sectionId)
      .map((g) => g.placeId ?? 0),
  );
  ...
  for (const field of fixed.fields!) {
    ...
    const existingNodes = fixed.layoutNodes!.filter(
      (n) => n.gridId === optionsGridId,
    );
