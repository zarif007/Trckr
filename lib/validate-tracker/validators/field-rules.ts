...
export function validateFieldRules(
  ...
  for (const [path, rules] of Object.entries(fieldRulesV2)) {
    ...
    if (dotIdx < 1) {
      warnings.push(
        `fieldRulesV2: key "${path}" must be "gridId.fieldId" format`,
      );
      continue;
    }
    ...
    if (!ctx.gridIds.has(gridId)) {
      warnings.push(
        `fieldRulesV2: key "${path}" references unknown grid "${gridId}"`,
      );
    }
    if (!ctx.fieldPaths.has(path)) {
      warnings.push(
        `fieldRulesV2: key "${path}" references unknown field "${fieldId}" in grid "${gridId}"`,
      );
    }
    ...
    for (const rule of rules) {
      if (!rule.id) {
        ...
        warnings.push(
          `fieldRulesV2["${path}"]: duplicate rule id "${rule.id}"`,
        );
      } else {
      ...
      if (!rule.outcome) {
        warnings.push(
          `fieldRulesV2["${path}"] rule "${rule.id}": missing "outcome" expression`,
        );
      }
