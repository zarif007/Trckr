...
export function validateBindings(ctx: ValidationContext): ValidatorResult {
  ...
  const builtInFunctionIds = new Set<string>(
    KNOWN_DYNAMIC_OPTIONS_FUNCTION_IDS as readonly string[],
  );
  ...
  for (const [functionId, definition] of Object.entries(customFunctions)) {
    ...
    if ("graph" in fn) {
      const httpNodes = fn.graph.nodes.filter(
        (node) => node.kind === "source.http_get",
      );
  ...
  for (const [fieldPath, entry] of Object.entries(ctx.bindings)) {
    ...
    if (!foreignSource) {
      if (!optGridId || !ctx.gridIds.has(optGridId)) {
        warnings.push(
          `Binding "${fieldPath}": optionsGrid "${entry.optionsGrid}" not found`,
        );
      }
    } else if (!optGridId) {
      warnings.push(
        `Binding "${fieldPath}": optionsGrid is required for cross-tracker binding`,
      );
    }
    ...
    if (!foreignSource) {
      if (!labelParsed.fieldId || !ctx.fieldIds.has(labelParsed.fieldId)) {
        warnings.push(
          `Binding "${fieldPath}": labelField "${entry.labelField}" not found`,
        );
    ...
    const valueMapping = (entry.fieldMappings ?? []).find(
      (m) => m.to === fieldPath,
    );
    if (!valueMapping) {
      warnings.push(
    ...
    for (const mapping of entry.fieldMappings ?? []) {
      ...
      if (!foreignSource) {
        if (!fromParsed.fieldId || !ctx.fieldIds.has(fromParsed.fieldId)) {
          warnings.push(
            `Binding "${fieldPath}": source field "${mapping.from}" not found`,
          );
        }
      }
      if (!toParsed.fieldId || !ctx.fieldIds.has(toParsed.fieldId)) {
        warnings.push(
          `Binding "${fieldPath}": target field "${mapping.to}" not found`,
        );
      }
    }
  }
  ...
  for (const field of ctx.fields) {
    if (
      ...
      if (!functionId) {
        warnings.push(
          `Dynamic select field "${field.id}" is missing config.dynamicOptionsFunction`,
        );
      } else if (
        !builtInFunctionIds.has(functionId) &&
        !customFunctionIds.has(functionId)
      ) {
        warnings.push(
