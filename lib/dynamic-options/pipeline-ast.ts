...
function nodeToSourceStep(
  node: DynamicFunctionGraphNode,
): DynamicPipelineSourceStep | null {
  switch (node.kind) {
    case "source.grid_rows":
      return {
        kind: "source.grid_rows",
        config: { gridId: node.config.gridId },
      };
...
function nodeToTransformStep(
  node: DynamicFunctionGraphNode,
): DynamicPipelineTransformStep | null {
  switch (node.kind) {
    ...
    case "transform.map_fields":
      return {
        ...
        config: { mappings: node.config.mappings },
      };
 ...
 */
export function graphToPipelineAst(graph: DynamicFunctionGraphDef): {
  ast?: DynamicPipelineAst;
  error?: string;
} {
  ...
  for (const edge of graph.edges) {
    const list: DynamicFunctionGraphEdge[] =
  ...
  while (currentId) {
    ...
    const outgoing: DynamicFunctionGraphEdge[] =
      outgoingByNode.get(currentId) ?? [];
  ...
  if (!sourceStep) {
    return {
      error: `First step after start must be a source node, got ${path[0].kind}`,
    };
  }
 ...
 */
export function pipelineAstToGraph(
  ast: DynamicPipelineAst,
): DynamicFunctionGraphDef {
