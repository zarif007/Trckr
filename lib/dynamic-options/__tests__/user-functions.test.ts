...
function makeContext(
  overrides?: Partial<DynamicOptionsContext>,
  ...
  return {
    ...
    sections: [{ id: "main_section", tabId: "overview_tab" }],
    gridData: {
      tasks_grid: [
...
describe("dynamic options user functions", () => {
            ...
            graph: {
              ...
              nodes: [
                ...
                },
                {
                  ...
                  position: { x: 600, y: 0 },
                  config: {
                    mapping: { label: "status", value: "status", id: "status" },
                  },
                },
              ],
              edges: [
  ...
  it("validates graph compile with missing return path", () => {
    ...
    expect(
      compile.errors.some((issue) => issue.message.includes("reachable")),
    ).toBe(true);
  });
  ...
  it("executes http_get source server-side with connector/secret auth", async () => {
    const fetcher = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      return {
        ...
        status: 200,
        text: async () =>
          JSON.stringify({ items: [{ code: "USD" }, { code: "EUR" }] }),
      } as Response;
    });
    ...
    const result = await executeDynamicOptionFunction({
      definition: {
        ...
        transforms: [
          { kind: "sort", by: "code", direction: "asc", valueType: "string" },
        ],
        output: { label: "code", value: "code", id: "code" },
        enabled: true,
      },
