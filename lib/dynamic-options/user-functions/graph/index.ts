...
const graphReferenceCache = new WeakMap<
  DynamicOptionGraphFunctionDef,
  CompiledDynamicFunctionPlan
>();
...
export function compileDynamicOptionFunctionGraph(
  ...
  if (!entryNode) {
    pushIssue(issues, {
      message: `entryNodeId "${graph.entryNodeId}" does not exist`,
    });
  ...
  if (!returnNode) {
    pushIssue(issues, {
      message: `returnNodeId "${graph.returnNodeId}" does not exist`,
    });
  ...
  const executionOrder = topo.filter((nodeId) =>
    reachableFromEntry.has(nodeId),
  );
  ...
  const plan: CompiledDynamicFunctionPlan = {
    ...
    incomingByNodeId,
    outgoingByNodeId,
    requiresRemote: reachableNodes.some((node) =>
      SERVER_ONLY_NODE_KINDS.has(node.kind),
    ),
    usesRuntimeRow: reachableNodes.some(
      (node) => node.kind === "source.current_context",
    ),
  };
