import { compareValues } from "@/lib/insights-query";
...
function readSelector(
  ...
  if ("fromContext" in selector)
    return getByPath(context, selector.fromContext);
  return undefined;
}
...
function resolvePredicateExpected(
  ...
  if (predicate.valueFromContext)
    return getByPath(context, predicate.valueFromContext);
...
function applyDslTransforms(
  ...
  for (const transform of definition.transforms ?? []) {
    ...
    if (transform.kind === "map_fields") {
      current = applyMapFieldsTransform(
        current,
        transform.mappings,
        args,
        context,
...
function buildLayoutRows(
  ...
  if (context.layoutNodes?.length) {
    for (const node of context.layoutNodes) {
      ...
      if (
        excludeSharedTab &&
        (section?.tabId === "shared_tab" ||
          section?.tabId === "master_data_tab")
      )
        continue;
      rows.push({
        gridId: grid.id,
...
async function executeHttpGetRequest(
  ...
  if (
    connector.allowHosts?.length &&
    !connector.allowHosts.includes(url.host)
  ) {
    warnings.push(
      `Host "${url.host}" is not allowlisted for connector "${connector.id}"`,
    );
    return null;
  }
  ...
  if (connector.auth.type === "secret_ref") {
    ...
    if (!resolver) {
      warnings.push(
        "Missing secret resolver for connector auth type secret_ref",
      );
      return null;
    }
  ...
  try {
    ...
    return source.responsePath
      ? getByPath(payload, source.responsePath)
      : payload;
  } finally {
...
function normalizeCurrentContext(
  node: Extract<DynamicFunctionGraphNode, { kind: "source.current_context" }>,
  context: DynamicOptionsContext,
): Record<string, unknown> {
...
async function executeGraphFunction(
  ...
  const compiled = compileDynamicOptionFunctionGraph(
    definition,
  ...
  for (const nodeId of compiled.plan.executionOrder) {
    ...
    const inputValue =
      incoming.length > 0 ? valuesByNodeId.get(incoming[0]!.source) : undefined;
    ...
    if (node.kind === "source.grid_rows") {
      valuesByNodeId.set(
        node.id,
        normalizeRows(context.gridData?.[node.config.gridId] ?? []),
      );
      continue;
    }
    ...
    if (node.kind === "source.http_get") {
      const payload = await executeHttpGetRequest(
        node.config,
        context,
        args,
        options,
    ...
    if (node.kind === "transform.unique") {
      valuesByNodeId.set(
        node.id,
        applyUniqueTransform(normalizeRows(inputValue), node.config.by),
      );
      continue;
    }
    ...
    if (node.kind === "transform.limit") {
      valuesByNodeId.set(
        node.id,
        normalizeRows(inputValue).slice(0, Math.max(0, node.config.count)),
      );
      continue;
    }
    ...
    if (node.kind === "transform.flatten_path") {
      valuesByNodeId.set(
        node.id,
        applyFlattenPathTransform(inputValue, node.config.path),
      );
      continue;
    }
    ...
    if (node.kind === "ai.extract_options") {
      ...
      if (!extractor) {
        warnings.push(
          "AI extractor is not configured for ai.extract_options node",
        );
        valuesByNodeId.set(node.id, []);
      } else {
