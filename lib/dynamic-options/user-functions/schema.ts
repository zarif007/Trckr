...
export const dynamicFunctionGraphSchema = z
  ...
  .superRefine((graph, ctx) => {
    ...
    const returnNode = graph.nodes.find(
      (node) => node.id === graph.returnNodeId,
    );
...
export const dynamicConnectorSchema = z
  .object({
    ...
    auth: z.discriminatedUnion("type", [
      ...
      z
        .object({ type: z.literal("secret_ref"), secretRefId: nonEmptyString })
        .strict(),
    ]),
...
export type DynamicOptionFunctionSchema = z.infer<
  typeof dynamicOptionFunctionSchema
>;
export type DynamicConnectorSchema = z.infer<typeof dynamicConnectorSchema>;
export type DynamicOptionsDefinitionsSchema = z.infer<
  typeof dynamicOptionsDefinitionsSchema
>;
