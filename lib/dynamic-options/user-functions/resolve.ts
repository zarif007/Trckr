import { getDynamicOptions, getRegisteredDynamicOptionsIds } from "../registry";
import type {
...
function buildContextVersion(context: DynamicOptionsContext): string {
  const payload = {
    ...
    layoutNodes: (context.layoutNodes ?? []).map(
      (n) => `${n.gridId}.${n.fieldId}`,
    ),
...
function buildRuntimeVersion(
  runtime: DynamicOptionsRuntimeContext | undefined,
): string {
  if (!runtime) return "";
  try {
...
export async function resolveDynamicOptions(
  ...
  const ttlSeconds =
    cacheTtlSecondsOverride ?? definition.cache?.ttlSeconds ?? 300;
  ...
  const { expiresAt } = setCachedDynamicOptions(
    cacheKey,
    result,
    ttlSeconds,
    nowMs,
