...
export function maxNumericRowId(rows: Array<Record<string, unknown>>): number | null {
...
export function allocateRowIdBetween(
  ...
  if (span < Number.EPSILON * Math.max(Math.abs(left), Math.abs(right), 1) * 1e6) {
...
export function assignOrderKeyAfterRowMove(
  ...
  const left = movedToIndex > 0 ? neighborOrderKey(rows[movedToIndex - 1]!) : null
  const right =
