...
import {
  getOptionsGridRowsForBinding,
  type ForeignGridDataBySchemaId,
} from "./binding-grid-data";
...
function buildOptionRowIndex(
  ...
  for (const row of rows) {
    ...
    const rowId =
      (row as { row_id?: unknown }).row_id ?? (row as { id?: unknown }).id;
    if (rowId !== undefined) {
      byId.set(rowId, row);
...
export function findOptionRow(
  ...
  if (!gridId) {
    console.warn(
      `[Bindings] Invalid binding - optionsGrid: "${binding.optionsGrid}"`,
    );
    return undefined;
  }
  if (!valueFieldId) {
    console.warn(
      `[Bindings] No value mapping for "${selectFieldPath}". Falling back to label/id matching.`,
    );
  }

  const rows = getOptionsGridRowsForBinding(
    binding,
    gridData,
  ...
  if (valueFieldId != null) {
    foundRow =
      index.byValue.get(selectedValue) ??
      index.byValue.get(String(selectedValue));
  }
  ...
  if (!foundRow && labelFieldId != null) {
    ...
    if (foundRow) {
      debugLog("Matched option row by label field", {
        labelFieldId,
        selectedValue,
      });
    }
  ...
  if (!foundRow) {
    ...
    if (selectedString.startsWith("opt-")) {
      ...
      if (!Number.isNaN(idx) && idx >= 0 && idx < index.byIndex.length) {
        ...
        debugLog("Matched option row by fallback opt-* index", {
          idx,
          selectedValue,
        });
      }
  ...
  if (!foundRow) {
    foundRow =
      index.byId.get(selectedValue) ?? index.byId.get(String(selectedValue));
    if (foundRow) {
...
export function getFullOptionRows(
  ...
  const gridId = normalizeOptionsGridId(binding.optionsGrid);
  if (!gridId) return [];
  return getOptionsGridRowsForBinding(
    binding,
    gridData,
    foreignGridDataBySchemaId,
  );
