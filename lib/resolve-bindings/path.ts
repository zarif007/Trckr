...
export function parsePath(path: string): ParsedPath {
  ...
  console.warn(
    `[Bindings] Invalid path format (expected 1-2 parts, or 3 for legacy): "${path}"`,
  );
  return { tabId: null, gridId: null, fieldId: null };
}
...
export function normalizeOptionsGridId(
  optionsGrid: string | undefined,
): string | undefined {
  if (!optionsGrid) return undefined;
  return optionsGrid.includes(".")
    ? optionsGrid.split(".").pop()!
    : optionsGrid;
}
