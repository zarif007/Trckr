...
export function applyBindings(
  ...
  for (const mapping of binding.fieldMappings ?? []) {
    if (mapping.to === selectFieldPath) {
      debugLog(
        `Skipping value mapping: ${mapping.from} -> ${mapping.to} (select field itself)`,
      );
      continue;
    }
    ...
    if (!(sourceFieldId in optionRow)) {
      ...
      debugLog(
        `Missing source field "${sourceFieldId}" for mapping ${mapping.from} -> ${mapping.to}`,
      );
      continue;
    }
    ...
    debugLog(
      `Adding update: ${mapping.from} (${sourceFieldId}=${optionRow[sourceFieldId]}) -> ${mapping.to}`,
    );
  }
