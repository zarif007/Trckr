...
const constExpr = (value: unknown): ExprNode =>
  ({ op: "const", value }) as ExprNode;
const fieldExpr = (fieldId: string): ExprNode =>
  ({ op: "field", fieldId }) as ExprNode;
...
describe("resolveFieldRulesForRow", () => {
  ...
  it("applies visibility rule — true means visible", () => {
    const map: FieldRulesMap = {
      "tasks_grid.title": [
        makeRule({ property: "visibility", outcome: constExpr(true) }),
      ],
    };
  ...
  it("applies required rule", () => {
    const map: FieldRulesMap = {
      "tasks_grid.title": [
        makeRule({ property: "required", outcome: constExpr(true) }),
      ],
    };
    const result = resolveFieldRulesForRow(map, "tasks_grid", {}, 0);
  ...
  it("applies disabled rule", () => {
    const map: FieldRulesMap = {
      "tasks_grid.status": [
        makeRule({ property: "disabled", outcome: constExpr(false) }),
      ],
    };
    const result = resolveFieldRulesForRow(map, "tasks_grid", {}, 0);
  ...
  it("applies label rule", () => {
    const map: FieldRulesMap = {
      "tasks_grid.notes": [
        makeRule({ property: "label", outcome: constExpr("Remarks") }),
      ],
    };
    const result = resolveFieldRulesForRow(map, "tasks_grid", {}, 0);
  ...
  it("skips disabled rules", () => {
    const map: FieldRulesMap = {
      "tasks_grid.title": [
        makeRule({ enabled: false, outcome: constExpr(true) }),
      ],
    };
    const result = resolveFieldRulesForRow(map, "tasks_grid", {}, 0);
  ...
  it("skips rules where condition evaluates falsy", () => {
    const map: FieldRulesMap = {
      "tasks_grid.title": [
        makeRule({ condition: constExpr(false), outcome: constExpr(true) }),
      ],
    };
    const result = resolveFieldRulesForRow(map, "tasks_grid", {}, 0);
    expect(result.overrides["title"]).toBeUndefined();
  ...
  it("evaluates condition against rowValues", () => {
    ...
    const withBlocked = resolveFieldRulesForRow(
      map,
      "tasks_grid",
    ...
    const withoutBlocked = resolveFieldRulesForRow(
      map,
      "tasks_grid",
  ...
  it("all lifecycle triggers are evaluated", () => {
    const triggers: Array<FieldRule["trigger"]> = [
      "onMount",
      "onRowCreate",
      "onRowCopy",
      "onRowFocus",
