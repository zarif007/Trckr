...
export function enrichBindingsFromSchema<T extends TrackerLike>(tracker: T): T {
  ...
  for (const [fieldPath, entry] of Object.entries(bindings)) {
    ...
    const valueFieldId = valueMapping
      ? parsePathFieldId(valueMapping.from)
      : labelFieldId;
    const reserved = new Set(
      [labelFieldId, valueFieldId].filter(Boolean) as string[],
    );
    ...
    const newMappings: Array<{ from: string; to: string }> = [
      ...(entry.fieldMappings ?? []),
    ];
