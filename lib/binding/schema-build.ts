...
export function getOptionGridLabelAndValueFieldIds(
  ...
  const hasLabel = fieldIds.find(
    (id) => id.endsWith("_label") || id.endsWith("_opt_label"),
  );
  const hasValue = fieldIds.find(
    (id) => id.endsWith("_value") || id.endsWith("_opt_value"),
  );
...
function ensureOptionsGridForField(
  ...
  const gridPlaceId =
    Math.max(
      0,
      ...grids
        .filter((g) => g.sectionId === container.sectionId)
        .map((g) => g.placeId ?? 0),
    ) + 1;
  ...
  if (!existingNodes.some((n) => n.fieldId === optionFieldId)) {
    layoutNodes = [
      ...layoutNodes,
      { gridId: optionsGridId, fieldId: optionFieldId, order: 1 },
    ];
  }
...
export function buildBindingsFromSchema<T extends TrackerLike>(tracker: T): T {
  ...
  const masterDataScope = resolveMasterDataScopeFromTracker(
    tracker as { masterDataScope?: unknown },
  );
  const hasLegacySharedTab = (fixed.tabs ?? []).some(
    (t) => t.id === LEGACY_SHARED_TAB_ID,
  );
  ...
  const refreshOptionGridMeta = () => {
    for (const grid of fixed.grids ?? []) {
      ...
      const meta = getOptionGridLabelAndValueFieldIds(
        grid.id,
        fixed.layoutNodes ?? [],
      );
  ...
  for (const field of fixed.fields!) {
    ...
    if (foreignSource) {
      if (existing?.labelField) {
        ...
        const hasValueMapping = existing.fieldMappings?.some(
          (m) => m.to === fieldPath,
        );
        fixed.bindings![fieldPath] = {
          ...
          ...(existing.optionsSourceKey
            ? { optionsSourceKey: existing.optionsSourceKey }
            : {}),
          fieldMappings: hasValueMapping
    ...
    if (optionGridMeta[optionsGridId]) {
      ...
      const existingGrid = (fixed.grids ?? []).find(
        (g) => g.id === optionsGridId,
      );
      if (existingGrid) {
        const meta = getOptionGridLabelAndValueFieldIds(
          optionsGridId,
    ...
    const valueMapping = {
      from: `${optionsGridId}.${valueFieldId}`,
      to: fieldPath,
    };
  ...
  if (hasOptionGrids && useLocalOptions) {
    ...
    if (!hasContainerTab) {
      const maxPlaceId = Math.max(
        0,
        ...(fixed.tabs ?? []).map((t) => t.placeId ?? 0),
      );
      fixed.tabs = [
        ...(fixed.tabs ?? []),
        {
