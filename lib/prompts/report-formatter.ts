...
- **segmentMarkdownTablesByColumn:** optional. Set to \`__gridId\` when multiple grids should appear as **separate tables** (one heading per grid). Set to \`__label\` when each instance should be its own table. Omit for a single combined table.
- **ops:** ordered transforms.

Ops:
- drop_columns: remove internal keys the user did not ask for (__dataId, __rowIndex, etc.). Keep __label when the user needs instance identity in the export.
